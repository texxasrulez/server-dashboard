...
class GenericLinuxAdapter implements DashboardAdapterInterface {
  ...
  protected function hint(string $key, array $allow, string $fallback): string {
    ...
    if ($raw !== '' && in_array($raw, $allow, true)) return $raw;
    return $fallback;
  }

  public function getCapabilities(): array {
    $panel = $this->hint('panel', ['hestia', 'none'], 'none');
    $web = $this->hint('web', ['nginx', 'apache', 'both', 'none'], $this->detectWebStack());
  ...
  public function getBackupJobs(): array {
    ...
    if (is_array($cfgJobs)) {
      foreach ($cfgJobs as $row) {
        if (is_string($row) && trim($row) !== '') $jobs[] = ['name' => trim($row)];
        if (is_array($row) && !empty($row['name'])) $jobs[] = ['name' => (string)$row['name']];
      }
    }
    return $jobs;
  }
  ...
  protected function detectWebStack(): string {
    ...
    if ($hasNginx && $hasApache) return 'both';
    if ($hasNginx) return 'nginx';
    if ($hasApache) return 'apache';
    return 'none';
  }
  ...
  protected function detectMta(): string {
    ...
    if ($hasExim) return 'exim';
    if ($hasPostfix) return 'postfix';
    return 'none';
  }
