...
class HestiaAdapter extends GenericLinuxAdapter {
  public function getCapabilities(): array {
    ...
    $caps['panel'] = 'hestia';
    $caps['adapter'] = 'hestia';
    if ($caps['mta'] === 'none') $caps['mta'] = 'exim';
    if ($caps['web'] === 'none') $caps['web'] = 'nginx';
    return $caps;
  }
  ...
  public function getWebDomains(): array {
    ...
    foreach ($users as $user) {
      ...
      foreach ($lines as $line) {
        ...
        if (!is_array($parts) || empty($parts[0])) continue;
        $candidate = strtolower(trim((string)$parts[0]));
        if (!preg_match('/^[a-z0-9][a-z0-9.\-]{0,251}[a-z0-9]$/', $candidate)) continue;
