 ...
 <div class="card">
  ...
  <div class="card-actions" style="margin-top:.75rem">
    <div class="row gap wrap">
      ...
      <button id="btnBackupAlerts" class="btn" type="button">Backup alerts</button>
      <button id="btnRestoreAlerts" class="btn danger" type="button">Restore alerts</button>
    </div>
  ...
  <div class="card">
    ...
    <table id="rulesTable" class="table js-sortable" style="width:100%; display:none">
      
      <thead>
        <tr>
          <th data-sort="str">Status</th>
          <th data-sort="str">Name</th>
          <th data-sort="str">Service</th>
          <th data-sort="str">Metric</th>
          <th data-sort="str">Condition</th>
          <th data-sort="str">Notify</th>
          <th data-sort="str">Severity</th>
          <th data-sort="num">Cooldown</th>
          <th data-sort="num">Last triggered</th>
          <th data-sort="num">Times</th>
          <th data-sort="str">Actions</th>
          <th data-sort="str">Select</th>
        </tr>
      </thead>
...
(function(){
  ...
  const URLS = {
    ...
    backup: '<?= h(project_url('/api/config_backup.php?type=alerts')) ?>',
    restore: '<?= h(project_url('/api/config_backup.php?type=alerts&restore_latest=1')) ?>'
  };
  ...
  const btnBackup = document.getElementById('btnBackupAlerts');
  const btnRestore = document.getElementById('btnRestoreAlerts');
  ...
  function renderRow(a){
    ...
    const lastTriggeredMs = coerceTs(a.last_triggered);
    const cooldownMin = Number(a.cooldown_min || 0);
    const timesTriggered = Number(a.times_triggered || 0);
    return `<tr data-id="${escapeHtml(a.id)}">
      <td data-value="${escapeHtml(a.enabled ? (silencedMs && silencedMs > Date.now() ? 'muted' : 'active') : 'disabled')}">${renderStatusPill(a, silencedMs)}</td>
      <td data-value="${escapeHtml(a.name||'')}">${escapeHtml(a.name||'')}</td>
      <td data-value="${escapeHtml(a.service_name||a.service_id||'')}">${escapeHtml(a.service_name||a.service_id||'')}</td>
      <td data-value="${escapeHtml(a.metric||'')}">${escapeHtml(a.metric||'')}</td>
      <td data-value="${escapeHtml(cond)}">${escapeHtml(cond)}</td>
      <td data-value="${escapeHtml(notify.join(', '))}">${notify.length ? escapeHtml(notify.join(', ')) : '-'}</td>
      <td data-value="${escapeHtml(a.severity||'')}">${escapeHtml(a.severity||'')}</td>
      <td data-value="${escapeHtml(String(cooldownMin))}">${escapeHtml(String(cooldownMin))} min</td>
      <td data-value="${escapeHtml(String(lastTriggeredMs || 0))}">${renderLastCell(a.last_triggered)}</td>
      <td data-value="${escapeHtml(String(timesTriggered))}">${escapeHtml(String(timesTriggered))}</td>
  ...
  btnBackup?.addEventListener('click', function(e){
    ...
    apiFetch(URLS.backup, {
      method:'POST',
      credentials:'same-origin',
      body: body
    }).then(r=>r.json()).then(resp=>{
      if (!resp || resp.error || !resp.ok){ throw new Error(resp?.error || 'Backup failed'); }
      const file = resp.path || resp.alerts_path || 'alerts backup';
      const dir = resp.backup_dir || 'config/backups';
      toastSuccess('Alerts backup created: ' + file + ' (' + dir + ')');
    }).catch((e)=>{
      toastError('Alerts backup failed: ' + (e?.message || 'unknown'));
    }).finally(()=>{
  ...
  btnRestore?.addEventListener('click', function(e){
    e.preventDefault();
    if (!confirm('Restore the latest alerts backup? This will overwrite the current alert rules.')) return;
    btnRestore.disabled = true;
    const body = new URLSearchParams();
    const token = csrfToken();
    if (token) body.set('_csrf', token);
    body.set('restore_latest', '1');
    apiFetch(URLS.restore, {
      method:'POST',
      credentials:'same-origin',
      body: body
    }).then(r=>r.json()).then(resp=>{
      if (!resp || resp.error || !resp.ok){ throw new Error(resp?.error || 'Restore failed'); }
      loadRules();
      const restored = resp.restored_from || resp.path || 'latest alerts backup';
      toastSuccess('Alerts restored from: ' + restored);
    }).catch((e)=>{
      toastError('Alerts restore failed: ' + (e?.message || 'unknown'));
    }).finally(()=>{
