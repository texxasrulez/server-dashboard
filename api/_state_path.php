...
if (!function_exists('dashboard_state_path')) {
    function dashboard_state_path(string $filename): string
    {
        ...
        if (!$chosen) {
            $chosen = $candidates[0] ?? ($root . '/data');
        }
        return rtrim($chosen, '/') . '/' . ltrim($filename, '/');
    }
}
if (!function_exists('dashboard_atomic_write')) {
    function dashboard_sync_default_metadata(string $path, ?string $source = null): void
    {
...
if (!function_exists('dashboard_atomic_write')) {
    function dashboard_atomic_write(string $path, string $contents): array
    {
        ...
        if ($bytes === false) {
            return ['ok' => false,'error' => 'write_failed','path' => $tmp];
        }
        dashboard_sync_default_metadata($tmp, $metadataSource);
        if (!@rename($tmp, $path)) {
            @unlink($tmp);
