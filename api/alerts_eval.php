...
function _noise_key_from_items($items)
{
    $arr = [];
    foreach ($items as $r) {
        $arr[] = [strtolower($r['name'] ?? ''), strtolower($r['status'] ?? '')];
    }
    ...
    if ($fh) {
        while (($line = fgets($fh)) !== false) {
            ...
            foreach ($items as $it) {
                ...
                if (!isset($agg[$k])) {
                    $agg[$k] = ['name' => $it['name'] ?? 'unknown','status' => $it['status'] ?? '','count' => 0];
                }
                $agg[$k]['count']++;
            }
        }
...
if ($probe) {
    ...
    if (is_file($probe_script)) {
        try {
            ob_start();
            include $probe_script;
            $probe_out = @ob_get_clean();
        } catch (Throwable $e) {
            @ob_end_clean();
            $probe_out = '';
        }
...
foreach ($rules as &$r) {
    ...
    if ($metric === 'status') {
        ...
        $v = isset($svc['latency_ms']) ? floatval($svc['latency_ms']) : null;
    } elseif ($metric === 'http_code') {
        $v = isset($svc['http_code']) ? intval($svc['http_code']) : null;
    } elseif ($metric === 'packet_loss_pct') {
        $v = isset($svc['packet_loss_pct']) ? floatval($svc['packet_loss_pct']) : null;
    }
    if ($v === null) {
        continue;
    }
    ...
    if ($r['consecutive_count'] >= $consecutive && $cool_ok && $pass) {
        ...
        if (!$dry) {
            ...
            if ($email) {
                ...
                if (!$__SUPPRESS) {
                    $res = Mailer::send($email, $sub, $body, []);
                } else {
...
if (!$dry && count($fired)) {
    ...
    if ($fh) {
        foreach ($fired as $ev) {
            fwrite($fh, json_encode($ev, JSON_UNESCAPED_SLASHES) . "\n");
        }
        fclose($fh);
    }
}
