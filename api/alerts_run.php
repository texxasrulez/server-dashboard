...
if (!$enabled) {
    echo json_encode(['ok' => false,'error' => 'alerts disabled']);
    exit;
}
if ($need !== '') {
    if (!hash_equals($need, (string)$token)) {
        http_response_code(401);
        echo json_encode(['ok' => false,'error' => 'bad token']);
        exit;
    }
} else {
    if (!cron_token_is_valid($token)) {
        http_response_code(401);
        echo json_encode(['ok' => false,'error' => 'bad token']);
        exit;
    }
...
if ($quiet !== '') {
    ...
    if (count($parts) === 2) {
        ...
        if ($start && $end) {
            ...
            if ($in) {
                echo json_encode(['ok' => true,'suppressed' => true,'reason' => 'quiet hours']);
                exit;
            }
        }
    }
...
$bad = array_values(array_filter($items, function ($r) use ($rank, $minRank) {
    return $rank($r['status'] ?? 'warn') >= $minRank;
}));
...
if (!$__SUPPRESS && count($bad) > 0) {
    ...
    if ($em !== '') {
        ...
        $lines = [];
        foreach ($bad as $r) {
            $lines[] = sprintf('- %s: %s (%s)', $r['name'], $r['status'], $r['details'] ?? '');
        }
