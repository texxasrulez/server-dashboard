...
function _noise_maybe_send_daily_digest()
{
    ...
    if ($fh) {
        while (($line = fgets($fh)) !== false) {
            ...
            $items =
                isset($row["items"]) && is_array($row["items"])
                    ? $row["items"]
                    : [];
            foreach ($items as $it) {
                $k = strtolower(
                    ($it["name"] ?? "unknown") . "|" . ($it["status"] ?? ""),
                );
                if (!isset($agg[$k])) {
                    $agg[$k] = [
    ...
    if (!count($agg)) {
        try {
            write_text_atomic($lastFile, (string) time());
        } catch (\Throwable $e) {
        }
        return;
    }
...
$bad = array_values(
    array_filter($items, function ($r) use ($rank, $minRank) {
