...
try {
    if (!function_exists('require_admin')) {
        throw new Exception('admin guard missing');
    }
    require_admin();
    ...
    if ($to === '') {
        ...
        if (is_array($cfg)) {
            ...
            if ($to === '' && isset($cfg['security']) && is_array($cfg['security']) && !empty($cfg['security']['admin_emails']) && is_array($cfg['security']['admin_emails'])) {
                ...
                if ($first) {
                    $to = trim((string)$first);
                }
            }
            if ($to === '' && isset($cfg['mail']) && is_array($cfg['mail']) && !empty($cfg['mail']['mail_from'])) {
                $to = Mailer::extractEmail((string)$cfg['mail']['mail_from']);
            }
        }
    }
    if ($to === '') {
        throw new InvalidArgumentException("Missing 'to' recipient for test send");
    }

    // Optional overrides
    $opts = array();
    if (isset($input['from'])) {
        $opts['from'] = (string)$input['from'];
    }
    if (isset($input['reply_to'])) {
        $opts['reply_to'] = (string)$input['reply_to'];
    }
