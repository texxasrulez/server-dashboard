...
//
// Modes:
//  POST/GET (default): snapshot local.json into <LOCAL_DIR>/backups/config-YYYYMMDD-HHMMSS.json and related alerts/services backups
//  ?type=alerts: snapshot alerts.json only into <LOCAL_DIR>/backups/alerts-YYYYMMDD-HHMMSS.json
//  ?type=alerts&restore_latest=1: restore the latest alerts backup into data/alerts.json
...
function requested_backup_type(): string
{
    $type = strtolower(trim((string) ($_REQUEST['type'] ?? 'config')));
    return in_array($type, ['config', 'alerts'], true) ? $type : 'config';
}

function backup_pattern_for_type(string $type): string
{
    return $type === 'alerts' ? 'alerts-*.json' : 'config-*.json';
}

function primary_alerts_target_path(string $root): string
{
    return $root . '/data/alerts.json';
}
...
try {
    ...
    $backupType = requested_backup_type();
    $primaryPattern = backup_pattern_for_type($backupType);
    ensure_dir($backupDir);
    migrate_legacy_backups($backupDir, legacy_backup_dirs($root));
    $base = [
      'backup_dir' => rel_backup_dir($root, $backupDir),
      'type' => $backupType,
    ];
    ...
    $list = list_backups($backupDir, $primaryPattern);
    ...
    if (isset($_GET['restore_latest']) || isset($_POST['restore_latest'])) {
        if ($backupType !== 'alerts') {
            http_response_code(400);
            resp(false, array_merge($base, ['error' => 'restore_latest currently supports type=alerts only']));
        }
        if (empty($list)) {
            http_response_code(404);
            resp(false, array_merge($base, ['error' => 'No alerts backup found']));
        }
        $latestBackup = $list[0];
        $restoreJson = @file_get_contents($latestBackup);
        if ($restoreJson === false) {
            throw new Exception('restore read failed');
        }
        $decoded = json_decode($restoreJson, true);
        if (!is_array($decoded)) {
            throw new Exception('restore invalid JSON');
        }

        $alertsTarget = primary_alerts_target_path($root);
        ensure_dir(dirname($alertsTarget));
        $snapshotPath = null;
        if (is_readable($alertsTarget)) {
            $snapshotPath = $backupDir . '/alerts-pre-restore-' . date('Ymd-His') . '.json';
            if (@copy($alertsTarget, $snapshotPath)) {
        ...
        if (file_put_contents($alertsTarget, $restoreJson, LOCK_EX) === false) {
            throw new Exception('restore write failed');
        }
        @chmod($alertsTarget, 0640);
        ...
        AuditLog::record(
            ...
            [
                ...
                'snapshot_path' => $snapshotPath ? basename($snapshotPath) : null,
            ],
            'alerts restore completed',
            'backup'
        );
    ...
    if (isset($_GET['prune']) || isset($_POST['prune'])) {
        ...
        if ($backupType !== 'alerts' && count($alertsList) > $keep) {
        ...
        if ($backupType !== 'alerts' && count($servicesList) > $keep) {
    ...
    if ($backupType === 'alerts') {
        if (!$alertsSrc) {
            throw new Exception('alerts.json not found (checked data/alerts.json, state/alerts.json)');
        }
    ...
    resp(true, array_merge($base, ['path' => basename((string) $dst), 'alerts_path' => $alertsPath, 'services_path' => $servicesPath, 'keep' => $keep, 'deleted' => $deleted]));
} catch (Throwable $e) {
