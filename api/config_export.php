...
function should_redact_key(string $key): bool
{
    $k = strtolower($key);
    if (in_array($k, ['api_tokens', 'csrf_secret', 'cron_token'], true)) {
        return true;
    }
    if (strpos($k, 'password') !== false) {
        return true;
    }
    if ($k === 'pass' || substr($k, -5) === '_pass') {
        return true;
    }
...
function redact_tree($value, ?string $key = null)
{
    if ($key !== null && should_redact_key($key)) {
        if (is_array($value)) {
            $out = [];
...
if ($redact) {
    if (is_array($out['config'])) {
        $out['config'] = redact_tree($out['config']);
    ...
    if (is_array($out['security'])) {
        $out['security'] = redact_tree($out['security']);
    }
