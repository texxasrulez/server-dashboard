...
function provider_guess(string $address): string
{
    ...
    if (preg_match('~(^|\\.)yahoo\\.~i', $host)) {
        return 'yahoo';
    }
    if (preg_match('~(outlook|live|hotmail|msn)\\.com$~i', $host)) {
        return 'microsoft';
    }
    return 'other';
}
...
function accounts_from_config(): array
{
    ...
    $acc = array_values(array_filter($acc, function ($a) {
        return !is_array($a) || !array_key_exists('enabled', $a) || $a['enabled'];
    }));
...
function gmail_unread_count(string $address): array
{
    ...
    // Prefer INBOX unread count (matches what users expect as inbox unread).
    ...
    if ($res !== false && $http < 400) {
        ...
        if (is_array($j) && isset($j['messagesUnread'])) {
            return ['ok' => true,'unseen' => (int)$j['messagesUnread'],'web' => 'https://mail.google.com/mail/u/0/#inbox'];
        }
    }

    // Fallback to a search estimate if label API is unavailable.
    $ch2 = curl_init('https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=1&q=' . rawurlencode('in:inbox is:unread'));
