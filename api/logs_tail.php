...
guard_api(['key' => 'logs_tail','require_token' => false,'type' => 'json']);
...
if ($idx >= 0 && $idx < count($list)) {
    $s = (string)$list[$idx];
    if (strpos($s, '|') !== false) {
        list($chosen_label, $path) = array_map('trim', explode('|', $s, 2));
    } else {
        $path = trim($s);
        $chosen_label = basename($path);
    }
}
if (!$path && $label !== '') {
    foreach ($list as $s) {
        ...
        $p = $s;
        if (strpos($s, '|') !== false) {
            list($lab, $p) = array_map('trim', explode('|', $s, 2));
        } else {
            $p = trim($s);
            $lab = basename($p);
        }
...
if (!$fh) {
    ...
    echo json_encode(['ok' => false,'error' => 'cannot open','path' => $path]);
    exit;
}
