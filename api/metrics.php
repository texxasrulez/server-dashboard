...
function get_disks()
{
    ...
    if ($rows) {
        $out = [];
        foreach ($rows as $r) {
            $out[] = ['mount' => $r['mount'], 'total' => (int)$r['total'], 'used' => (int)$r['used'], 'free' => (int)$r['avail'], 'pct_used' => (float)$r['pct']];
        }
        return $out;
    }
...
function check_tcp($host, $port, $timeout = 2)
{
    $start = microtime_ms();
    $fp = @fsockopen($host, (int)$port, $errno, $errstr, $timeout);
    $lat = microtime_ms() - $start;
    if (!$fp) {
        return ['status' => 'down','latency_ms' => $lat];
    } fclose($fp);
...
function check_http($host, $port, $path = '/', $timeout = 3, $tls = false)
{
    ...
    return ['status' => $ok ? 'up' : 'down','latency_ms' => $lat,'http_code' => $code];
}
function check_ping($host, $timeout = 1)
{
...
function annotate_service($svc)
{
    ...
    $port = (int)($svc['port'] ?? 80);
    $check = strtolower($svc['check'] ?? 'tcp');
    $path = $svc['path'] ?? '/';
    ...
    if ($check === 'http') {
        $r = check_http($host, $port, $path);
    } elseif ($check === 'ping') {
        $r = check_ping($host);
    } else {
        $r = check_tcp($host, $port);
    }
    $res = array_merge($res, $r);
    if (($res['status'] ?? '') === 'up' && ($res['latency_ms'] ?? 0) > 800) {
        $res['status'] = 'warn';
    }
    return $res;
}
