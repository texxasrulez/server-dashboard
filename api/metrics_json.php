...
guard_api(['key' => 'metrics_json','require_token' => true,'type' => 'json']);
...
if ($cores <= 0) {
    $cores = (int)($_SERVER['NUMBER_OF_PROCESSORS'] ?? 1);
}
if ($cores <= 0) {
    $cores = 1;
}
...
"]];
            if ($method === 'https') {
                $opts['ssl'] = ['verify_peer' => true,'verify_peer_name' => true,'SNI_enabled' => true,'allow_self_signed' => true];
            }
