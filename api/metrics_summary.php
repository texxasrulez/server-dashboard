        ...
        if (@is_dir($hwmonRoot)) {
            ...
            if (is_array($entries)) {
                foreach ($entries as $dir) {
                    ...
                    if (!preg_match('/(k10temp|coretemp|zenpower|cpu)/i', $name)) {
                        continue;
                    }
...
function pick_disk_path()
{
    ...
    if (!$chosen && defined('DISK_METRICS_PATH') && DISK_METRICS_PATH) {
        ...
        if (@is_dir(DISK_METRICS_PATH)) {
            $chosen = DISK_METRICS_PATH;
        }
    }
    // fallbacks
    $candidates = ['/'];
...
if (@is_readable('/proc/cpuinfo')) {
    ...
    $cpu_cores = 0;
    foreach ($ci as $ln) {
        if (preg_match('/^processor\s*:\s*\d+$/', trim($ln))) {
            $cpu_cores++;
        }
    }
    if ($cpu_cores <= 0) {
...
if (empty($processes) && @is_readable('/proc')) {
    ...
    $memTotalKB = 0;
    foreach ($meminfo as $ln) {
        if (preg_match('/^MemTotal:\s+(\d+)\s+kB$/', $ln, $m)) {
            $memTotalKB = (int)$m[1];
            break;
        }
    }
    ...
    foreach (@glob('/proc/[0-9]*/status') as $status) {
        ...
        foreach (@file($status, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [] as $ln) {
            if (preg_match('/^VmRSS:\s+(\d+)\s+kB$/', $ln, $m)) {
                $vmrss = (int)$m[1];
                break;
            }
        }
    ...
    function __net_counters()
    {
        ...
        if (is_array($dev)) {
            ...
            foreach ($dev as $iface => $v) {
                ...
                if (strpos($name, 'docker') === 0 || strpos($name, 'veth') === 0 || strpos($name, 'br-') === 0) {
                    continue;
                }
                $rx += (int)($v['rx_bytes'] ?? 0);
                $tx += (int)($v['tx_bytes'] ?? 0);
