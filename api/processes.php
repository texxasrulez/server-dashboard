...
function proc_api_default_limit(): int {
  ...
  if (!is_numeric($raw)) return PROC_API_DEFAULT_LIMIT;
  $n = (int)$raw;
  if ($n < PROC_API_MIN_LIMIT) return PROC_API_MIN_LIMIT;
  if ($n > PROC_API_MAX_LIMIT) return PROC_API_MAX_LIMIT;
  return $n;
}
...
function proc_api_cache_ttl(): int {
  ...
  if (!is_numeric($raw)) return PROC_API_CACHE_TTL;
  $n = (int)$raw;
  if ($n < 1) return 1;
  if ($n > 10) return 10;
  return $n;
...
function proc_api_clean_user($raw): string {
  $s = trim((string)$raw);
  if ($s === '') return '';
  if (!preg_match('/^[a-z_][a-z0-9_.-]{0,31}$/i', $s)) return '';
  return $s;
}

function proc_api_cache_file(): string {
  $base = defined('STATE_DIR') ? (string)STATE_DIR : (dirname(__DIR__) . '/state');
...
function proc_api_cache_get(string $key): ?array {
  if (proc_api_cache_available()) {
    ...
    if ($ok && is_array($value)) return $value;
  }
...
function proc_api_uid_map(): array {
  ...
  $passwd = '/etc/passwd';
  if (!is_readable($passwd)) return $map;
...
function proc_api_read_total_cpu_ticks(): int {
  ...
  if ($line === null || strpos($line, 'cpu ') !== 0) return 0;
  $parts = preg_split('/\s+/', trim($line));
  if (!is_array($parts) || count($parts) < 2) return 0;
  $sum = 0;
  for ($i = 1; $i < count($parts); $i++) {
    if (is_numeric($parts[$i])) $sum += (int)$parts[$i];
  }
  return $sum;
}
...
function proc_api_uptime_seconds(): ?float {
  ...
  if (!is_array($parts) || empty($parts[0]) || !is_numeric($parts[0])) return null;
...
/**
 * @return array{uid:int,vmrss_kb:int,threads:int}
 */
function proc_api_parse_status(string $path): array {
  ...
  if (!is_array($lines)) return ['uid' => 0, 'vmrss_kb' => 0, 'threads' => 0];

  foreach ($lines as $line) {
    if (strpos($line, 'Uid:') === 0) {
      $parts = preg_split('/\s+/', trim(substr($line, 4)));
      if (is_array($parts) && isset($parts[0]) && is_numeric($parts[0])) $uid = (int)$parts[0];
    } elseif (strpos($line, 'VmRSS:') === 0) {
      if (preg_match('/VmRSS:\s+(\d+)/', $line, $m)) $rss = (int)$m[1];
    } elseif (strpos($line, 'Threads:') === 0) {
      if (preg_match('/Threads:\s+(\d+)/', $line, $m)) $threads = (int)$m[1];
    }
  }
...
/**
 * @return array{read_bytes:int|null,write_bytes:int|null}
 */
function proc_api_parse_io(string $path): array {
  if (!is_readable($path)) return ['read_bytes' => null, 'write_bytes' => null];
  $read = null;
  $write = null;
  $lines = @file($path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
  if (!is_array($lines)) return ['read_bytes' => null, 'write_bytes' => null];
  foreach ($lines as $line) {
    if (strpos($line, 'read_bytes:') === 0) {
      $v = trim(substr($line, strlen('read_bytes:')));
      if (is_numeric($v)) $read = (int)$v;
    } elseif (strpos($line, 'write_bytes:') === 0) {
      $v = trim(substr($line, strlen('write_bytes:')));
      if (is_numeric($v)) $write = (int)$v;
    }
  }
...
function proc_api_sample_processes(): array {
  ...
  foreach ($entries as $entry) {
    ...
    if (!is_string($statRaw) || $statRaw === '') continue;

    $l = strrpos($statRaw, ')');
    if ($l === false) continue;
    $comm = trim(substr($statRaw, strpos($statRaw, '(') + 1, $l - strpos($statRaw, '(') - 1));
    $rest = trim(substr($statRaw, $l + 1));
    $parts = preg_split('/\s+/', $rest);
    if (!is_array($parts) || count($parts) < 22) continue;
    ...
    if ($threads <= 0) $threads = $threadsFromStat;
    ...
    if ($io['read_bytes'] !== null) $row['io_read_bytes'] = (int)$io['read_bytes'];
    if ($io['write_bytes'] !== null) $row['io_write_bytes'] = (int)$io['write_bytes'];
...
function proc_api_apply_cpu_delta(array $current, array $previous, int $totalDelta): array {
  if ($totalDelta <= 0) return $current;
  ...
  uasort($rows, static function(array $a, array $b) use ($sort, $desc): int {
    switch ($sort) {
      ...
      case 'user':
        ...
        if ($cmp === 0) $cmp = ((int)$a['pid']) <=> ((int)$b['pid']);
        break;
      case 'cmd':
      ...
      case 'mem':
        ...
        if ($cmp === 0) $cmp = ((float)$a['cpu_pct']) <=> ((float)$b['cpu_pct']);
        break;
      case 'cpu':
      default:
        $cmp = ((float)$a['cpu_pct']) <=> ((float)$b['cpu_pct']);
        if ($cmp === 0) $cmp = ((int)$a['rss_kb']) <=> ((int)$b['rss_kb']);
        break;
    }
    return $desc ? -$cmp : $cmp;
  });
...
function proc_api_filter_rows(array $rows, string $filter, string $user): array {
  ...
  if ($needle === '' && $userNeedle === '') return $rows;

  $out = [];
  foreach ($rows as $row) {
    $rowUser = strtolower((string)($row['user'] ?? ''));
    if ($userNeedle !== '' && $rowUser !== $userNeedle) continue;
...
$sort = proc_api_sort_key((string)($_GET['sort'] ?? 'cpu'));
$limit = proc_api_limit($_GET['limit'] ?? proc_api_default_limit());
$filter = proc_api_clean_substr($_GET['filter'] ?? '', PROC_API_FILTER_MAX);
$userFilter = proc_api_clean_user($_GET['user'] ?? '');
