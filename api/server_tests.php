...
if (!function_exists('status_weight')) {
    function status_weight($st)
    {
        ...
        if ($s === 'ok' || $s === 'good' || $s === 'pass' || $s === 'success' || $s === 'up' || $s === 'info') {
            return 1.0;
        }
        if ($s === 'warn' || $s === 'warning' || $s === 'medium' || $s === 'notice') {
            return 0.6;
        }
...
if (!function_exists('st_get_cpu_cores')) {
    function st_get_cpu_cores()
    {
        ...
        if ($n <= 0 && !empty($_SERVER['NUMBER_OF_PROCESSORS'])) {
            $n = (int)$_SERVER['NUMBER_OF_PROCESSORS'];
        }
        if ($n <= 0) {
            $n = 1;
        }
    ...
    function scan_services($override = null)
    {
                    ...
                    foreach ($data['items'] as $svc) {
                        if (!empty($svc['enabled'])) {
                            ...
                            if ($check === 'http') {
                                ...
                                if ($host !== '' && $port > 0) {
                                    $targets[] = $host.':'.$port.'|' . ($name ?: ($host.':'.$port));
                                }
                            }
                        }
                    }
        ...
        foreach ($targets as $s) {
            ...
            if (stripos($spec, 'http://') === 0 || stripos($spec, 'https://') === 0) {
                ...
                $timeoutForTarget = $timeout;
                if (isset($extra['timeout_ms'])) {
                    $timeoutForTarget = max(0.1, ((int)$extra['timeout_ms']) / 1000.0);
                } elseif (isset($extra['timeout'])) {
                    $timeoutForTarget = max(0.1, (float)$extra['timeout']);
                }

                $expectCode = isset($extra['status']) ? (int)$extra['status'] : 0;
                $contains = isset($extra['contains']) ? (string)$extra['contains'] : '';
                $headers = [
                    'User-Agent: ServerDiag/1',
                    'Accept: */*',
                    'Connection: close',
                    'Host: ' . $host,
                ];
                ...
                foreach ($headerLines as $hdrLine) {
                    if (preg_match('/^HTTP\/\S+\s+(\d{3})\b/', (string)$hdrLine, $m2)) {
                        $code = (int)$m2[1];
                    }
                }
                $okCode = false;
                ...
                $okContains = true;
                if ($contains !== '') {
                    $okContains = (is_string($body) && strpos($body, $contains) !== false);
                }

                $status = 'fail';
                if ($okCode && $okContains) {
                    $status = 'ok';
                ...
                if ($code >= 300 && $code < 400) {
                    $location = '';
                    foreach ($headerLines as $hdrLine) {
                        if (stripos((string)$hdrLine, 'Location:') === 0) {
                            $location = trim(substr((string)$hdrLine, 9));
                        }
                    }
                    if ($location !== '') {
                        $details .= ' redirect';
                    ...
                    $details .= ' reachable but access restricted';
                } elseif ($code === 404) {
                    $details .= ' reachable but path missing';
                }
                if ($contains !== '') {
                    $details .= ' contains="'.(strlen($contains) > 32 ? substr($contains, 0, 29).'...' : $contains).'"';
                }
                $out[] = ['name' => $name, 'status' => $status, 'details' => $details];
            } else {
                // TCP connect check
                $hp = $spec;
                $host = $hp;
...
if ($audit) {
    ...
    @mkdir($state.'/logs', 0775, true);
    dashboard_log_append($state.'/logs/diag_audit.log', 'server_tests', 'diagnostic action', [
        'action' => $__action,
...
function check_tls_cert($host)
{
    ...
    if (!$host) {
        return ['name' => 'TLS cert','status' => 'warn','details' => 'no host'];
    }
    ...
    if (!$fp) {
        return ['name' => 'TLS cert','status' => 'warn','details' => 'unreachable'];
    }
    $params = stream_context_get_params($fp);
    @fclose($fp);
    if (empty($params['options']['ssl']['peer_certificate'])) {
        return ['name' => 'TLS cert','status' => 'warn','details' => 'no cert'];
    }
    $cert = $params['options']['ssl']['peer_certificate'];
    $info = openssl_x509_parse($cert);
    $to = $info['validTo_time_t'] ?? null;
    if (!$to) {
        return ['name' => 'TLS cert','status' => 'warn','details' => 'invalid cert'];
    }
...
function check_inodes_for($path)
{
    ...
    if (!$out) {
        return ['name' => 'Inodes free','status' => 'warn','details' => 'unavailable'];
    }
    $lines = preg_split('/\r?\n/', trim($out));
    if (count($lines) < 2) {
        return ['name' => 'Inodes free','status' => 'warn','details' => 'unparsed'];
    }
...
function st_resolve_command($cmd)
{
    $cmd = trim((string)$cmd);
    if ($cmd === '') {
        return null;
    ...
    $resolved = is_string($out) ? trim($out) : '';
    if ($resolved !== '' && (@is_file($resolved) || @is_link($resolved))) {
        return $resolved;
    }
    return null;
}

function st_exec_readonly($binary, $args = '')
{
    $path = $binary;
    if (strpos((string)$binary, '/') === false) {
        $path = st_resolve_command($binary);
    }
    if (!is_string($path) || $path === '' || !@is_file($path)) {
        return [null, null];
    }

    $cmd = $path . ($args !== '' ? ' ' . $args : '') . ' 2>&1';
    ...
    $out = @shell_exec($cmd);
    return [is_string($out) ? trim($out) : null, null];
}

function st_apt_lists_age_seconds()
{
    ...
    foreach ($it as $entry) {
        if ($entry === '.' || $entry === '..' || $entry === 'lock' || $entry === 'partial') {
            continue;
        }
        $path = $dir . '/' . $entry;
        if (!@is_file($path)) {
    ...
    return $latest > 0 ? max(0, time() - $latest) : null;
}

function st_security_updates_response($status, $details)
{
    return ['name' => 'Security updates', 'status' => $status, 'details' => $details];
}
...
function check_security_updates()
{
    ...
    $classifyCount = static function ($count) use ($warn, $fail) {
        if ($count <= 0) {
            return 'ok';
        }
        if ($count >= $fail) {
    ...
    $isAptFamily = preg_match('/(debian|ubuntu|linuxmint|pop|elementary|neon)/i', $osrel)
        || st_resolve_command('apt-get')
        || st_resolve_command('apt');
    if ($isAptFamily) {
        $aptCheck = '/usr/lib/update-notifier/apt-check';
        if (!@is_file($aptCheck)) {
            $aptCheck = st_resolve_command('apt-check');
        }
        if (is_string($aptCheck) && $aptCheck !== '') {
            list($out,) = st_exec_readonly($aptCheck);
            if (is_string($out) && preg_match('/^\s*(\d+)\s*;\s*(\d+)\s*$/m', trim($out), $m)) {
                $total = (int)$m[1];
                $security = (int)$m[2];
                if ($security <= 0 && $total <= 0) {
                    return st_security_updates_response('ok', '0 pending updates');
                }
                $details = $security . ' security updates';
                if ($total > $security) {
                    $details .= ', ' . $total . ' total pending';
                }
                return st_security_updates_response($classifyCount($security), $details);
            }
        }

        list($out,) = st_exec_readonly('apt-get', '-s upgrade');
        if (is_string($out) && $out !== '') {
            $total = preg_match_all('/^Inst\s+/m', $out, $m);
            if ($total === 0) {
                $age = st_apt_lists_age_seconds();
                if ($age !== null && $age > 7 * 86400) {
                    return st_security_updates_response('warn', '0 pending updates, package lists stale');
                }
                return st_security_updates_response('ok', '0 pending updates');
            }

            $security = preg_match_all('/^Inst\s+.*(?:security|debian-security|ubuntu(?:[- ](?:esm|pro))?)/mi', $out, $m);
            if ($security > 0) {
                $details = $security . ' security updates';
                if ($total > $security) {
                    $details .= ', ' . $total . ' total pending';
            ...
            return st_security_updates_response('warn', $total . ' pending updates (security classification unavailable)');
        }

        return st_security_updates_response('warn', 'APT present but update status unavailable');
    }

    $dnfPath = st_resolve_command('dnf');
    if ($dnfPath || preg_match('/(rhel|centos|fedora|rocky|almalinux)/i', $osrel)) {
        list($out, $code) = st_exec_readonly($dnfPath ?: 'dnf', '-q check-update --security');
        if ($code === 0) {
            return st_security_updates_response('ok', '0 security updates');
        }
        if ($code === 100 && is_string($out)) {
            $lines = array_values(array_filter(preg_split('/\r?\n/', trim($out)), function ($line) {
                $line = trim((string)$line);
                if ($line === '') {
                    return false;
                }
                return (bool)preg_match('/^[A-Za-z0-9_.:+-]+\s+\S+\s+\S+$/', $line);
            }));
            $count = count($lines);
            if ($count > 0) {
                return st_security_updates_response($classifyCount($count), $count . ' security updates');
            }
        }
        return st_security_updates_response('warn', 'DNF present but security update status unavailable');
    }

    $yumPath = st_resolve_command('yum');
    if ($yumPath) {
        list($out, $code) = st_exec_readonly($yumPath, '--security check-update');
        if ($code === 0) {
            return st_security_updates_response('ok', '0 security updates');
        }
        if ($code === 100 && is_string($out)) {
        ...
        return st_security_updates_response('warn', 'YUM present but security update status unavailable');
    }

    $pacman = st_resolve_command('checkupdates');
    if ($pacman) {
        list($out, $code) = st_exec_readonly($pacman);
        if (($code === 0 || $code === 2) && is_string($out)) {
            $count = count(array_filter(preg_split('/\r?\n/', trim($out))));
            if ($count === 0) {
                return st_security_updates_response('ok', '0 pending updates');
            }
            return st_security_updates_response('warn', $count . ' pending updates (security advisories not classified)');
        }
        return st_security_updates_response('warn', 'checkupdates present but update status unavailable');
    }

    if (st_resolve_command('pacman')) {
        list($out, $code) = st_exec_readonly('pacman', '-Qu');
        if (($code === 0 || $code === 1) && is_string($out)) {
            $count = count(array_filter(preg_split('/\r?\n/', trim($out))));
            if ($count === 0) {
                return st_security_updates_response('ok', '0 pending updates');
            }
        ...
        return st_security_updates_response('warn', 'pacman present but update status unavailable');
    }

    if (st_resolve_command('zypper')) {
        list($out, $code) = st_exec_readonly('zypper', '--non-interactive patch-check');
        if (is_string($out) && preg_match('/no needed patches found/i', $out)) {
            return st_security_updates_response('ok', '0 security patches');
        }
        if (is_string($out) && preg_match('/(\d+)\s+needed patch/i', $out, $m)) {
            $count = (int)$m[1];
            return st_security_updates_response($classifyCount($count), $count . ' security patches');
        }
        if ($code === 0) {
            return st_security_updates_response('ok', '0 security patches');
        }
        return st_security_updates_response('warn', 'zypper present but patch status unavailable');
    }

    if (@is_file('/etc/os-release')) {
        return st_security_updates_response('warn', 'package manager not supported for security classification');
    }

    return st_security_updates_response('warn', 'package manager unknown');
}
...
function se_firewall_present()
{
    ...
    // Helper: resolve command paths even when php/apache PATH omits /usr/sbin.
    $cmd_path = function ($cmd) {
        return st_resolve_command($cmd);
    };
    $run_cmd = function ($path, $args = '') {
        if (!is_string($path) || $path === '') {
            return '';
        }
        return (string)@shell_exec($path . ($args !== '' ? ' ' . $args : '') . ' 2>&1');
    };
    ...
    // 2) iptables family (legacy or nft shim): rules/chains imply active policy;
    //    permission/init errors imply presence under an unprivileged web user.
    foreach (['iptables', 'iptables-nft', 'iptables-legacy'] as $iptablesCmd) {
        $iptables = $cmd_path($iptablesCmd);
        if (!$iptables) {
            continue;
        }
        $s = $run_cmd($iptables, '-S');
        if (is_string($s) && preg_match('/^\\-A\\s+/m', $s)) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => $iptablesCmd . ' rules present'];
        }
        $l = $run_cmd($iptables, '-L -n');
        if (is_string($l) && preg_match('/^Chain\\s+\\w+\\s+\\(policy/i', $l)) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => $iptablesCmd . ' chains present'];
        }
        if (is_string($s.$l) && preg_match('/permission denied|operation not permitted|not authorized|failed to initialize nft|xtables lock/i', $s.$l)) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => $iptablesCmd . ' present (insufficient privileges)'];
        }
    }

    // 3) If an iptables-family binary exists at all, treat that as presence.
    // Many hosted/web runtimes block the actual rule listing even though the firewall is active.
    foreach (['iptables', 'iptables-nft', 'iptables-legacy'] as $iptablesCmd) {
        $iptables = $cmd_path($iptablesCmd);
        if ($iptables) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => $iptablesCmd . ' binary present'];
        }
    }

    // 4) Final on-disk evidence before declaring "not found".
    foreach ([
        '/usr/sbin/iptables',
        ...
        if (@file_exists($evidencePath) || @is_link($evidencePath)) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => basename($evidencePath) . ' present'];
        }
    }

    // 5) Kernel/runtime evidence for netfilter/iptables on developer laptops and hosts.
    foreach ([
        ...
        if (@file_exists($modulePath) || @is_dir($modulePath)) {
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => basename($modulePath) . ' module present'];
        }
    }
    $procModules = @file_get_contents('/proc/modules');
    if (is_string($procModules) && preg_match('/\b(ip_tables|ip6_tables|x_tables|nf_tables|nfnetlink)\b/i', $procModules)) {
        return ['name' => 'firewall status', 'status' => 'ok', 'details' => 'netfilter modules loaded'];
    }

    // 6) Kernel tables exposed in /proc also indicate iptables support/ruleset presence.
    foreach (['/proc/net/ip_tables_names', '/proc/net/ip6_tables_names'] as $procFile) {
        if (@is_file($procFile)) {
            $raw = @file_get_contents($procFile);
            if (is_string($raw) && trim($raw) !== '') {
                return ['name' => 'firewall status', 'status' => 'ok', 'details' => basename($procFile) . ' present'];
            }
            return ['name' => 'firewall status', 'status' => 'ok', 'details' => basename($procFile) . ' available'];
        }
    }

    // 7) ufw/firewalld services active => OK
    $systemctl = $cmd_path('systemctl');
    $ufw = $systemctl ? @trim($run_cmd($systemctl, 'is-active ufw')) : '';
    $fwd = $systemctl ? @trim($run_cmd($systemctl, 'is-active firewalld')) : '';
    if ($ufw === 'active' || $fwd === 'active') {
        return ['name' => 'firewall status', 'status' => 'ok', 'details' => ($ufw === 'active' ? 'ufw' : 'firewalld').' active'];
    }

    // 8) Config present on disk (may be inactive at runtime) => WARN (not fail)
    ...
    // 9) No evidence available without root/extra tools => UNKNOWN (don’t fail)
...
function se_ssh_rootlogin()
{
    ...
    if ($txt === false) {
        return ['name' => 'ssh root login', 'status' => 'unknown', 'details' => '/etc not readable'];
    }
    $m = null;
    if (preg_match('/^\s*PermitRootLogin\s+(\S+)/mi', $txt, $m)) {
        $v = strtolower($m[1]);
        if ($v === 'no' || $v === 'prohibit-password') {
            return ['name' => 'ssh root login', 'status' => 'ok', 'details' => $v];
        }
...
function se_world_writable()
{
    ...
    if (!$found) {
        return ['name' => 'world-writable dirs', 'status' => 'ok', 'details' => 'none under project/tmp'];
    }
...
switch ($action) {
    case 'security':   $items = scan_security();
        break;
    case 'filesystem': $items = scan_filesystem();
        break;
    case 'services':
        $override = [];
        if (isset($req['targets']) && is_array($req['targets'])) {
            $override = $req['targets'];
        }
        $items = scan_services($override);
        $score = score($items);
        break;
    case 'performance':$items = scan_performance();
        break;
    case 'quick':
    default:           $items = scan_quick();
        break;
}
