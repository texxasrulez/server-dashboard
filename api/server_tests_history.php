...
if (!$enabled) {
    echo json_encode(['ok' => true,'results' => [],'score' => 0]);
    exit;
}
...
function parse_number($name, $items)
{
    foreach ($items as $it) {
        if (($it['name'] ?? '') === $name) {
            $d = (string)($it['details'] ?? '');
            if ($name === 'Disk Free' && preg_match('/([0-9]+(?:\.[0-9]+)?)%/', $d, $m)) {
                return (float)$m[1];
            }
            if ($name === 'Load Avg (1m)' && is_numeric($d)) {
                return (float)$d;
            }
            if ($name === 'Security updates' && preg_match('/(\d+)/', $d, $m)) {
                return (int)$m[1];
            }
            if ($name === 'TLS cert expiry' && preg_match('/(-?\d+)/', $d, $m)) {
                return (int)$m[1];
            }
        }
    }
