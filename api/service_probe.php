...
if (!in_array($check, ['tcp','http','ping'])) {
    $check = 'tcp';
}
if ($check !== 'http') {
    $path = '/';
...
if ($check === 'http') {
    $res = check_http($host, $port, $path, $secs);
} elseif ($check === 'ping') {
    $res = check_ping($host, $secs);
} else {
    $res = check_tcp($host, $port, $secs);
}

if (($res['status'] ?? '') === 'up' && ($res['latency_ms'] ?? 0) > $timeout_ms) {
    $res['status'] = 'warn';
}
AuditLog::record(
    'service.probe',
