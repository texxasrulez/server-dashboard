...
if (!in_array($check, ['tcp','http','ping'], true)) {
    $check = 'tcp';
}
if ($check !== 'http') {
    $path = '/';
...
if (!in_array($type, ['app','db','mail','cache','other'], true)) {
    $type = 'other';
}
...
AuditLog::record(
    $found ? 'service.update' : 'service.create',
    (string)$entry['id'],
    true,
    [
        'name' => $entry['name'],
    ...
    $found ? 'service update ok' : 'service create ok',
    'service'
);
