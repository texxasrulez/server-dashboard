...
foreach ($items as $it) {
    ...
    if ($check === "http") {
        ...
        $r = check_ping($it["host"] ?? "", $timeout);
    } else {
        $r = check_tcp($it["host"] ?? "", (int) ($it["port"] ?? 0), $timeout);
    }
    if (
        ($r["status"] ?? "") === "up" &&
        ($r["latency_ms"] ?? 0) > ($it["timeout_ms"] ?? 800)
    ) {
        $r["status"] = "warn";
    }
    $r["id"] = $it["id"];
...
try {
    write_json_atomic($statePath, ["results" => $res, "ts" => time()]);
} catch (\Throwable $e) {
...
foreach ($res as $row) {
    ...
    try {
        append_line_locked($histFile, $line);
    } catch (\Throwable $e) {
        break;
    }
}
