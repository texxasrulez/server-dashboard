...
try {
    ...
    if (!$host) {
        resp(false, ['error' => 'missing host']);
    }

    if ($port <= 0) {
        $port = ($secure === 'ssl' || $transport === 'smtps') ? 465 : 25;
    }
    $timeout = 8;
    ...
    if (!$fp) {
        resp(false, ['connect' => ['ok' => false,'error' => trim($errstr),'code' => $errno],'server' => compact('host', 'port', 'secure', 'transport')]);
    }
