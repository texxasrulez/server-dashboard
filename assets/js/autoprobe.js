(function () {
  ...
  async function fetchServerCfg() {
    try {
      ...
      return {
        enabled: !!s.enabled,
        interval: Math.max(5, parseInt(s.interval || 60, 10)),
      };
    } catch (e) {
      return null;
    }
    ...
    savingTimer = setTimeout(async () => {
      try {
        await fetch(API_SET, {
          ...
          body: JSON.stringify({
            enabled: !!cfg.enabled,
            interval: Math.max(5, parseInt(cfg.interval || 60, 10)),
          }),
        });
      } catch (e) {}
    }, 250);
  ...
  async function tick() {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("services:summary", { detail: sum }),
      );
    } catch (e) {}
  }
  ...
  window.AutoProbe = {
    ...
    async set(cfg) {
      ...
      const next = {
        ...
        interval: Math.max(
          5,
          parseInt(cfg.interval || curr.interval || 60, 10),
        ),
      };
      writeLocal(next);
      saveServerCfg(next);
      window.dispatchEvent(
        new CustomEvent("autoprobe:config", { detail: next }),
      );
      start(next);
    },
    restart() {
  ...
  window.addEventListener("storage", (e) => {
    if (e.key === KEY) start(readCfgSync());
  });
  initCfg().then(start);
})();
