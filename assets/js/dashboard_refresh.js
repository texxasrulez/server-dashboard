(function () {
  ...
  window.Dashboard.refresh = async function () {
    ...
    safeFetchJSON("api/services_probe_all.php").then((data) => {
      if (data)
        window.dispatchEvent(
          new CustomEvent("services:probeUpdate", { detail: data }),
        );
    });
    ...
    if (
      window.IndexProcessMeters &&
      typeof window.IndexProcessMeters.update === "function"
    ) {
...
async function hideDisabledFromList() {
  try {
    ...
    document
      .querySelectorAll("[data-svc-id], [data-svc-name], .service-card")
      .forEach((card) => {
        ...
        if (shouldHide) card.classList.add("svc-disabled");
        else card.classList.remove("svc-disabled");
      });
  } catch (e) {}
}
...
(function () {
  async function fetchDisabled() {
    try {
      ...
      return { byId: new Set(), byName: new Set() };
    }
  }
  ...
  function matchCards() {
    return Array.from(
      document.querySelectorAll(
        "[data-svc-id], [data-svc-name], [data-svc], .service-card, .svc-card",
      ),
    );
  }
  ...
  async function hideDisabledServices() {
    ...
    matchCards().forEach((card) => {
      ...
      if (hide) card.setAttribute("hidden", "");
...
(function () {
  ...
  async function getEnabled() {
    try {
      const r = await fetch("api/services_enabled_ids.php", {
        cache: "no-store",
      });
      if (r.ok) {
        return await r.json();
  ...
  function queueHide() {
    ...
    hideTimer = setTimeout(applyHide, 80);
  }
  ...
  async function applyHide() {
    ...
    const enabledNames = new Set(
      (en.names || []).map((s) => String(s).toLowerCase()),
    );
    ...
    document.querySelectorAll(SEL).forEach((card) => {
      ...
      const name = (
        ...
        card.querySelector(".svc-name, .name")?.textContent ||
        ""
      )
        .trim()
        .toLowerCase();
      const host = (
        card.getAttribute("data-svc-host") ||
        card.dataset.svcHost ||
        card.querySelector("[data-svc-host]")?.textContent ||
        ""
      )
        .trim()
        .toLowerCase();
      const portRaw = (
        card.getAttribute("data-svc-port") ||
        card.dataset.svcPort ||
        card.querySelector("[data-svc-port]")?.textContent ||
        ""
      ).trim();
      const port = /^\d{1,5}$/.test(portRaw)
        ? portRaw
        : portRaw.match(/(\d{1,5})$/)?.[1] || "";
  ...
  if (!window.__applyHideWrap) {
    ...
    const _orig =
      (window.Dashboard && window.Dashboard.refresh) || async function () {};
    window.Dashboard = window.Dashboard || {};
    window.Dashboard.refresh = async function () {
      await _orig();
      queueHide();
