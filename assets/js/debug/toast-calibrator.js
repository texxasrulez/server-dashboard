(function () {
  "use strict";
  if (window.__TOAST_CAL__) {
    try {
      window.__TOAST_CAL__.show(true);
    } catch (e) {}
    return;
  }
  ...
  function cssPath(el) {
    ...
    while (el && el.nodeType === 1 && el !== document.body) {
      ...
      var cls = (el.className || "")
        ...
        .filter(Boolean);
      if (cls.length) {
        sel += "." + cls.slice(0, 3).join(".");
      }
      var sib = el,
        idx = 1;
      while ((sib = sib.previousElementSibling) && idx < 6) {
  ...
  function candidateList() {
    var nodes = Array.from(document.querySelectorAll("*")).filter(
      function (el) {
  ...
  function makeUI() {
    ...
    function render() {
      ...
      candidateList().forEach(function (n) {
        ...
        item.innerHTML =
          '<span class="sel">' +
          txt +
          '</span> <span class="tiny">→</span> <code>' +
          path +
          "</code>";
        ...
        item.addEventListener("click", function () {
          ...
          try {
            window.dispatchEvent(new CustomEvent("ui:toast-position"));
          } catch (e) {}
        });
    ...
    sh.getElementById("hide").addEventListener("click", function () {
    ...
    return {
      show: function (v) {
        host.style.display = v === false ? "none" : "block";
      },
      rescan: render,
    };
  }
