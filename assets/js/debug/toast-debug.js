(function () {
  "use strict";
  if (window.__toastdbg) {
    try {
      window.__toastdbg.show(true);
    } catch (e) {}
    return;
  }
  ...
  function uniq(arr) {
    return Array.prototype.filter.call(arr, function (x, i) {
      return arr.indexOf(x) === i;
    });
  }
  function qsAll() {
    return uniq(
      SELECTORS.flatMap(function (sel) {
  ...
  function makeOverlay() {
    ...
    posSel.value = (function () {
      try {
        return localStorage.getItem("ui.toast_position") || "bottom-center";
      } catch (e) {
    ...
    sh.getElementById("apply").addEventListener("click", function () {
      try {
        localStorage.setItem("ui.toast_position", posSel.value);
      } catch (e) {}
      applyCurrent(posSel.value);
      status("Applied & saved: " + posSel.value);
      try {
        window.dispatchEvent(
          new CustomEvent("ui:toast-position", {
            detail: { position: posSel.value },
          }),
        );
      } catch (e) {}
    });
    sh.getElementById("test").addEventListener("click", function () {
      if (window.toast && window.toast.notice)
        window.toast.notice("Toast debug: " + new Date().toLocaleTimeString());
      else {
        ...
        t.style.cssText =
          "pointer-events:auto;background:rgba(30,30,30,.95);color:#fff;padding:8px 10px;border-radius:8px;margin:6px;box-shadow:0 6px 20px rgba(0,0,0,.35)";
    ...
    function status(txt) {
      sh.getElementById("status").textContent = txt;
    }
    function renderList() {
