...
(function () {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.shiftKey && e.key.toLowerCase() === "d") toggle();
  });
  ...
  window.fetch = async function (input, init) {
    ...
    const traceOn =
      localStorage.getItem("dbg.trace") === "1" || params.get("trace") === "1";
    if (!traceOn) {
    ...
    if (isApi) {
      ...
      try {
        ...
        state.logs.unshift(
          `[${new Date().toLocaleTimeString()}] ${url} — ${res.status} ${dt}ms${info}`,
        );
  ...
  panel.addEventListener("click", async (e) => {
    if (e.target && e.target.id === "dbg-ping") {
      const t0 = performance.now();
      const r = await fetch("api/debug_ping.php?trace=1", {
        cache: "no-store",
      });
      const dt = (performance.now() - t0).toFixed(0);
      const j = await r.json();
      qs("#dbg-latency", panel).textContent =
        ` ${dt}ms (server ${Math.round(j?.trace?.elapsed_ms || 0)}ms)`;
    }
  });
