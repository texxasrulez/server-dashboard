...
(function () {
  ...
  function pickWebUrl(ac) {
    ...
    var domain = (function () {
      var _d = addr && addr.indexOf("@") > -1 ? addr.split("@")[1] : "";
    ...
    if (
      p === "microsoft" ||
      p === "outlook" ||
      /(outlook|live|hotmail|msn)\.com$/.test(domain)
    )
  ...
  function sumUnseen(j) {
    ...
    if (Array.isArray(j.accounts))
      return j.accounts.reduce(function (n, a) {
        return n + (a && a.unseen | 0);
      }, 0);
    return j.unseen | 0;
  }
  function accountsWithUnseen(j) {
    return j && Array.isArray(j.accounts)
      ? j.accounts.filter(function (a) {
          return a && (a.unseen | 0) > 0;
        })
      : [];
  }
  ...
  function tick() {
    var b = ensureBar();
    if (!b) return setTimeout(tick, 1000);
    fetch("api/email_status.php?_=" + Date.now(), {
      ...
      .then(function (txt) {
        ...
        if (!j) {
          ...
          return setTimeout(tick, POLL_ERR_MS);
        }
        if (j && j.enabled === false) {
          b.style.display = "none";
          return setTimeout(tick, POLL_OK_MS);
        }
        render(b, j);
        setTimeout(tick, POLL_OK_MS);
      })
      .catch(function () {
        setTimeout(tick, POLL_ERR_MS);
      });
  }
