...
(function () {
  ...
  function hasIndicator() {
    return !!(
      ...
      (window.EmailIndicator.init || window.EmailIndicator.start)
    );
  }
  ...
  (function boot() {
    ...
    if (!hasIndicator()) {
      chain = chain.then(function () {
        return load(base + "/email-indicator.js?v=" + stamp);
      });
    }
    chain = chain.then(function () {
      return load(base + "/email-indicator.toast.js?v=" + stamp);
    });
    chain
      .then(function () {
        try {
          if (window.EmailIndicator && window.EmailIndicator.init)
            window.EmailIndicator.init();
