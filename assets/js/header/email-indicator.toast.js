...
(function () {
  ...
  function getState(addr) {
    try {
      return sessionStorage.getItem(KEY_PREFIX + addr) || "";
    } catch (_) {
      return "";
    }
  }
  function setState(addr, val) {
    try {
      sessionStorage.setItem(KEY_PREFIX + addr, val);
    } catch (_) {}
  }
  ...
  function summarizeFail(entry) {
    ...
    if (p === "google" || p === "gmail")
      return (
        "Gmail notifier for " +
        addr +
        " needs attention (token missing or expired). Reconnect in Config → Email."
      );
    if (p === "imap")
      return (
        "IMAP notifier for " +
        addr +
        " failed to fetch unread mail. Check password/app password or server reachability."
      );
    return "Notifier for " + addr + " failed to fetch unread mail.";
  }

  function checkOnce() {
    return fetch("api/email_status.php?verbose=1&_=" + Date.now(), {
      ...
      .then(function (j) {
        ...
        j.accounts.forEach(function (ac) {
          ...
          if (state === "fail" && prev !== "fail") {
            ...
            toast(
              "success",
              "Email notifier reconnected for " +
                (ac.address || "account") +
                ".",
            );
  ...
  function start() {
    stop();
    checkOnce();
    timer = setInterval(checkOnce, POLL_MS);
  }
  function stop() {
    if (timer) {
      clearInterval(timer);
  ...
  window.addEventListener("email-accounts-changed", function () {
    clearAllStates();
    checkOnce();
  });
  ...
  function lateStart() {
    try {
      if (
        ...
        typeof window.EmailIndicator.init === "function"
      ) {
