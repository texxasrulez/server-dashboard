...
(function () {
  ...
  function log() {
    try {
      console.log.apply(console, ["[userbox]", ...arguments]);
    } catch (e) {}
  }
  function warn() {
    try {
      console.warn.apply(console, ["[userbox]", ...arguments]);
    } catch (e) {}
  }
  ...
  ready(function () {
    ...
    if (!box) {
      ...
      window.__UserMenu = {
        state: () => ({ error: "no .userbox", hasBtn: false, hasMenu: false }),
      };
      return;
    }
    ...
    // Find or create menu
    let menu =
      document.getElementById("usermenu") || box.querySelector(".usermenu");
    if (!menu) {
      menu = document.createElement("div");
      menu.className = "usermenu";
      menu.hidden = true;
      const base = (
        document.querySelector("body")?.getAttribute("data-api-metrics") ||
        location.pathname
      ).replace(/\/api\/.*$/, "");
...
(function () {
  try {
    var btn =
      document.getElementById("userbtn") || document.querySelector(".userbtn");
    var menu =
      btn && btn.closest(".userbox")
        ? btn.closest(".userbox").querySelector(".usermenu")
        : document.querySelector(".userbox .usermenu");
    if (btn) {
