...
(function () {
    ...
    for (var i = 0; i < cards.length; i++) {
      ...
      if (!svg.querySelector("#" + gid)) {
        ...
        for (var s = 0; s < stops.length; s++) {
          var st = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "stop",
          );
  ...
  async function tick() {
    ...
    try {
      ...
      setGauge(
        ...
        "cpu_load5",
        pct5,
        `load ${load5.toFixed(2)} / ${cores} core${cores > 1 ? "s" : ""}`,
        stateFromPct(pct5),
      );
      setGauge(
        root,
        "cpu_load15",
        pct15,
        `load ${load15.toFixed(2)} / ${cores} core${cores > 1 ? "s" : ""}`,
        stateFromPct(pct15),
      );
      ...
      const coresAvg =
        temps.cores && typeof temps.cores === "object"
          ? (function (obj) {
  ...
  try {
    Bus.dispatchEvent(new CustomEvent("cpu.ready"));
  } catch (_) {}
})();
