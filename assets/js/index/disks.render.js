...
(function () {
  ...
  if (!window.DashboardDebug.hideApiLinks)
    window.DashboardDebug.hideApiLinks = function () {
      document.querySelectorAll(".api-link").forEach((el) => el.remove());
    };
  try {
  ...
  function card(label, id) {
    return `<div class="disk-card" data-id="${id}">
        <div class="device">
  ...
  function stateFromUsage(p) {
    if (!isFinite(p)) return "ok";
    if (p >= 90) return "bad";
    if (p >= 75) return "warn";
    return "ok";
  }
  function stateFromFree(p) {
    if (!isFinite(p)) return "ok";
    if (p <= 10) return "bad";
    if (p <= 25) return "warn";
    return "ok";
  }
  function stateFromTemp(c) {
    if (c == null || !isFinite(c)) return "ok";
    if (c >= 80) return "bad";
    if (c >= 60) return "warn";
    return "ok";
  }
  ...
  async function tick() {
    ...
    try {
      ...
      setCard(
        ...
        "usage",
        usedPct,
        `Usage: ${isFinite(usedPct) ? usedPct.toFixed(0) : "--"}%`,
        stateFromUsage(usedPct),
      );
      if (totalGB > 0) {
      ...
      setCard(
        root,
        "temp",
        tempC != null ? Math.min(tempC, 100) : 0,
        `Temp: ${tempC != null ? tempC + "°C" : "n/a"}`,
        stateFromTemp(tempC),
      );
