...
(function () {
  ...
  function ensureSparkTip() {
    ...
    t.style.boxShadow = "0 4px 16px rgba(0,0,0,.35)";
    t.hidden = true;
    document.body.appendChild(t);
    return t;
  }
  function sparkShowTip(html, x, y) {
    var t = ensureSparkTip();
    t.innerHTML = html;
    t.hidden = false;
  ...
  const API =
    document.body && document.body.dataset && document.body.dataset.apiMetrics
  ...
  const ZOOM_IN_FACTOR = 0.8; // stronger zoom in
  ...
  function getCpuPercent(j) {
    let v =
      (j &&
        j.cpu &&
        (j.cpu.percent ?? j.cpu.percentage ?? j.cpu_pct ?? j.cpuPercent)) ??
  ...
  function getMemPercent(j) {
    ...
    const total_gb = num(
      j?.mem?.total_gb,
      isFinite(used_gb) && isFinite(free_gb) ? used_gb + free_gb : NaN,
    );
    if (isFinite(used_gb) && isFinite(total_gb) && total_gb > 0)
      return clamp((used_gb / total_gb) * 100, 0, 100);
    ...
    if (isFinite(used_b) && isFinite(total_b) && total_b > 0)
      return clamp((used_b / total_b) * 100, 0, 100);
  ...
  function getMemMeta(j) {
    ...
    if (isFinite(used_gb) && isFinite(total_gb) && total_gb > 0)
      return `${used_gb.toFixed(1)} GB / ${total_gb.toFixed(1)} GB`;
  ...
  function getUptime(j) {
    return String(j?.uptime ?? j?.system?.uptime ?? "");
  }
  ...
  class MiniChart {
    constructor(canvas, options = {}) {
      ...
      parent.addEventListener(
        "touchstart",
        (e) => {
          if (e.touches && e.touches.length === 1) this.onDown(e.touches[0]);
        },
        { passive: true },
      );
      parent.addEventListener(
        "touchmove",
        (e) => {
          if (e.touches && e.touches.length === 1) this.onMove(e.touches[0]);
        },
        { passive: true },
      );
    ...
    resetView() {
      this.viewCount = Math.min(
        300,
        Math.max(MIN_COUNT, this.values.length || 300),
      );
    ...
    push(v) {
      ...
      if (this.dynamicY) {
        const slice = this.values.slice(
          this.viewStart,
          this.viewStart + this.viewCount,
        );
    ...
    resize() {
      ...
      const w =
        this.canvas.clientWidth || this.canvas.parentElement.clientWidth || 300;
    ...
    onWheel(e) {
      ...
      const newCount = clamp(
        newCountRaw,
        minCount,
        Math.max(minCount, maxCount),
      );

      // keep cursor anchored
      const leftCount = Math.round(
        (centerIndex - this.viewStart) * (newCount / oldCount),
      );
      this.viewCount = newCount;
      this.viewStart = clamp(
        centerIndex - leftCount,
        0,
        Math.max(0, this.values.length - this.viewCount),
      );
    ...
    hover(e) {
      ...
      const visCount = Math.max(
        1,
        Math.min(this.viewCount, this.values.length),
      );
      const x = Math.max(0, Math.min(e.clientX - rect.left, w));
      const iVis = Math.round((visCount - 1) * (w ? x / w : 0));
      const idx = Math.max(
        0,
        Math.min(this.values.length - 1, this.viewStart + iVis),
      );
      const v = this.values[idx];
      const now = Date.now();
      const when = new Date(
        now - (this.values.length - 1 - idx) * 5000,
      ).toLocaleString();
  ...
  function ensureCharts() {
    if (!state.charts.load) {
      ...
      if (c1)
        state.charts.load = new MiniChart(c1, {
          label: "CPU Load",
          unit: "%",
          yMax: 100,
          dynamicY: false,
      ...
      if (c2)
        state.charts.mem = new MiniChart(c2, {
          label: "Memory Used",
          unit: "%",
          yMax: 100,
          dynamicY: false,
      ...
      if (c3)
        state.charts.net = new MiniChart(c3, {
          label: "Net KB/s",
          unit: "KB/s",
          yMax: 2000,
          dynamicY: true,
      ...
      const ovs = Array.from(
        document.querySelectorAll(".charts .chart .overlay"),
      );
      state.ov = {
        load: ovs[0] || null,
        mem: ovs[1] || null,
      ...
      for (const el of ovs)
        if (el) el.textContent = "wheel: zoom • drag: pan • dblclick: reset";
    }
  }
  ...
  async function sample() {
    ...
    try {
      ...
      if (state.ov?.load) {
        state.ov.load.innerHTML = `Uptime: ${up}<br>Load avg: ${loadStr}<br><span style="opacity:.7">wheel: zoom • drag: pan • dblclick: reset</span>`;
      }
      if (state.ov?.mem) {
        state.ov.mem.innerHTML = `Memory: ${memStr}<br><span style="opacity:.7">wheel: zoom • drag: pan • dblclick: reset</span>`;
      }
      if (state.ov?.net) {
        const nowKb =
          typeof kbs === "number" && isFinite(kbs)
            ? `${kbs.toFixed(1)} KB/s`
            : "—";
        state.ov.net.innerHTML = `Net: ${nowKb}<br><span style="opacity:.7">wheel: zoom • drag: pan • dblclick: reset</span>`;
      }
    } catch (e) {
  ...
  let timer = null;
  try {
    console.debug("API metrics endpoint", API.toString ? API.toString() : API);
  } catch (_) {}
  function start() {
    if (!timer) {
      sample();
