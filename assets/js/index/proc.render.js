...
(function () {
  ...
  function setGauge(root, id, pct, text, state) {
    ...
    if (p) {
      p.style.strokeDashoffset = String(100 - pct);
    }
    const m = el.querySelector('[data-role="meta"]');
    if (m) {
      m.textContent = text || "";
      m.classList.remove("updated");
      void m.offsetWidth;
      m.classList.add("updated");
    }
  ...
  function applyStale(root) {
    ...
    root.querySelectorAll(".proc-card").forEach((c) => {
      if (stale) c.setAttribute("data-stale", "1");
      else c.removeAttribute("data-stale");
    });
  }

  function toGB(bytes) {
    const n = Number(bytes) || 0;
    return n > 0 ? n / 1073741824 : 0;
  }
  const MB = 1024 * 1024;
  ...
  async function tick() {
    ...
    try {
      ...
      try {
        ...
        console.error("PROC fetch failed", err);
        const root = ensure(cardsList);
        if (root) showError(root, "network");
        return;
      }
      if (!r.ok) {
        console.error("PROC HTTP", r.status);
        const root = ensure(cardsList);
        if (root) showError(root, "HTTP " + r.status);
        return;
      }
      ...
      try {
        ...
        console.error("PROC JSON parse failed", err, jtxt?.slice?.(0, 180));
        const root = ensure(cardsList);
        if (root) showError(root, "json");
        return;
      }
      if (!loggedOnce) {
        try {
          console.debug("PROC JSON", j);
        } catch (_) {}
        loggedOnce = true;
      }
      ...
      const load1 =
        Number(
          j?.cpu?.load1 ?? (Array.isArray(j?.loadavg) ? j.loadavg[0] : 0),
        ) || 0;
      ...
      setGauge(
        ...
        "cpu",
        cpuPct,
        `load ${load1.toFixed(2)} / ${cores} core${cores > 1 ? "s" : ""}`,
        stateFromPct(cpuPct),
      );
      ...
      const total = toGB(totalB),
        used = toGB(usedB),
        free = toGB(freeB);
      const buffers = toGB(buffersB),
        cached = toGB(cachedB);
      ...
      const buffersLabel =
        buffersB <= 0
          ...
          : buffersB < MB
            ? "<1 MB"
            : Math.round(buffersB / MB) + " MB";
      const cachedLabel =
        cachedB < MB && cachedB > 0
          ? "<1 MB"
          : cachedB < 1024 ** 3
            ? Math.round(cachedB / MB) + " MB"
            : (cachedB / 1024 ** 3).toFixed(1) + " GB";
  ...
  function start() {
    ...
    if (!staleTimer) {
      ...
      staleTimer = setInterval(() => {
        const r = document.getElementById("procGrid");
        if (r) applyStale(r);
      }, 1000);
    }
  }
...
function __procGaugeInstall(root) {
  ...
  for (var i = 0; i < cards.length; i++) {
    ...
    var svg = cardEl.querySelector("svg.arc.gauge");
    if (!svg) continue;
    ...
    if (!svg.querySelector("#" + gid)) {
      var lg = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "linearGradient",
      );
      lg.setAttribute("id", gid);
      lg.setAttribute("x1", "10");
      lg.setAttribute("y1", "50");
      lg.setAttribute("x2", "90");
    ...
    if (!svg.querySelector(".needle")) {
      ...
      var blade = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "line",
      );
      blade.setAttribute("class", "blade");
      blade.setAttribute("x1", x1);
      blade.setAttribute("y1", y1);
      blade.setAttribute("x2", x2);
      blade.setAttribute("y2", y2);
      ...
      var hub = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "circle",
      );
      ...
      hub.setAttribute(
        "style",
        "fill: var(--border, transparent); stroke: var(--accent, transparent); stroke-width: 0",
      );
