(function () {
  ...
  function applyMetrics(data) {
    ...
    $$("[data-metric]").forEach((el) => {
      ...
      switch (key) {
        case "uptime":
          val = data.uptime_human || "";
          break;
        case "load1":
          val =
            data.loadavg && data.loadavg[0] != null
              ? Number(data.loadavg[0]).toFixed(3)
              : "";
          break;
        case "load5":
          val =
            data.loadavg && data.loadavg[1] != null
              ? Number(data.loadavg[1]).toFixed(3)
              : "";
          break;
        case "load15":
          val =
            data.loadavg && data.loadavg[2] != null
              ? Number(data.loadavg[2]).toFixed(3)
              : "";
          break;
        case "mem_used_human":
          val = (data.memory && data.memory.used_human) || "";
          break;
        case "mem_total_human":
          val = (data.memory && data.memory.total_human) || "";
          break;
        case "mem_used_pct":
          val =
            data.memory && data.memory.used_percent != null
              ? data.memory.used_percent + "%"
              : "";
          break;
        case "disk_used_human":
          val = (data.disk && data.disk.used_human) || "";
          break;
        case "disk_free_human":
          val = (data.disk && data.disk.free_human) || "";
          break;
        case "disk_total_human":
          val = (data.disk && data.disk.total_human) || "";
          break;
        case "disk_used_pct":
          val =
            data.disk && data.disk.used_percent != null
              ? data.disk.used_percent + "%"
              : "";
          break;
        case "disk_temp_c":
          val =
            data.disk && data.disk.temp_c != null
              ? data.disk.temp_c + "°C"
              : "";
          break;
        case "cpu_temp_package":
          val =
            data.cpu && data.cpu.temps && data.cpu.temps.package != null
              ? Number(data.cpu.temps.package).toFixed(1) + "°C"
              : "";
          break;
        case "cpu_temp_core_avg":
          if (data.cpu && data.cpu.temps && data.cpu.temps.cores) {
            const vals = Object.values(data.cpu.temps.cores)
              .map(Number)
              .filter((v) => !isNaN(v));
            if (vals.length) {
    ...
    if (memBar && data.memory && data.memory.used_percent != null) {
      ...
      const pctEl =
        memBar.closest(".proc-row") &&
        memBar.closest(".proc-row").querySelector(".pct");
    ...
    if (diskBar && data.disk && data.disk.used_percent != null) {
      ...
      const pctEl =
        diskBar.closest(".proc-row") &&
        diskBar.closest(".proc-row").querySelector(".pct");
      ...
      data.processes.forEach((p) => {
        const row =
          document.querySelector(
            '.proc-row[data-proc-name="' + p.name + '"]',
          ) ||
          Array.from(document.querySelectorAll(".proc-row")).find(
            (r) =>
              (r.querySelector(".name")?.textContent || "").trim() === p.name,
          );
        if (!row) return;
