(function () {
  ...
  function cardTemplate(item) {
    ...
    const latencyText =
      statusMeta.latency_ms != null ? statusMeta.latency_ms + " ms" : "";
    ...
    const uptimeTag =
      uptimeMeta && uptimeMeta.uptime_pct != null
        ...
        : "";
    const alertTag =
      alertMeta && alertMeta.last_alert
        ? `<span class="tag warn">alert ${relTime(alertMeta.last_alert.ts)}</span>`
        : "";
    const muteTag =
      alertMeta && alertMeta.silenced_until
        ? `<span class="tag muted">muted ${relTime(alertMeta.silenced_until)}</span>`
        : "";
    const historyHref = id
      ? `history.php?service=${encodeURIComponent(id)}`
      : "";
    const actionChips = [];
    if (historyHref) {
      actionChips.push(
    ...
    if (
      ...
      actionChips.push(
        `<a class="chip small warn svc-action" href="#" data-action="mute" data-rules="${alertMeta.rule_ids.join(",")}">Silence alerts</a>`,
      );
    }
    const actionsHtml = actionChips.length
      ? `<div class="svc-actions">${actionChips.join("")}</div>`
      : "";
  ...
  function handleCardActions(ev) {
    ...
    if (!rules.length) {
      window.toast &&
        window.toast.warn &&
        window.toast.warn("No alert rules found for this service.");
      return;
    }
