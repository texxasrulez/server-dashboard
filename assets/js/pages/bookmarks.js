...
(function () {
  ...
  function tOK(m) {
    try {
      toast && toast.success && toast.success(m);
    } catch (e) {}
  }
  function tWarn(m) {
    try {
      toast && toast.warn && toast.warn(m);
    } catch (e) {}
  }
  function tErr(m, sticky) {
    try {
      toast && toast.error && toast.error(m, { sticky: !!sticky });
    } catch (e) {}
  }
  function logI(ev, data) {
    try {
      clientLog && clientLog.info && clientLog.info(ev, data || {});
    } catch (e) {}
  }
  function logE(ev, data) {
    try {
      clientLog && clientLog.error && clientLog.error(ev, data || {});
    } catch (e) {}
  }
  function csrfToken() {
  ...
  function resolveCandidates(endpoint) {
    ...
    try {
      if (typeof project_url === "function")
        c.push(project_url("/api/" + endpoint));
    } catch (e) {}
  ...
  function resolveFaviconUrl(path) {
    ...
    if (p[0] === "/") {
      if (/^\/favicon_proxy\.php(?:\?|$)/i.test(p))
        return apiUrl(p.replace(/^\/+/, ""));
      return p;
    }
  ...
  function wireUI() {
    ...
    var c = $("#bmCancel");
    if (c) c.addEventListener("click", resetForm);
    var s = $("#bmSave");
    if (s)
    ...
    var so = $("#bmSort");
    if (so) so.addEventListener("change", renderList);
    var cSave = $("#catSave");
    if (cSave)
      cSave.addEventListener("click", function (ev) {
        ev.preventDefault();
        catSave();
      });
    var cCancel = $("#catCancel");
    if (cCancel)
    ...
    if (cDel)
      cDel.addEventListener("click", function (ev) {
        ...
        apiFetch(
          apiUrl(
            "bm_categories_delete.php?id=" +
              encodeURIComponent($("#catId").value),
          ),
          { method: "POST" },
        )
          .then(parseJsonResponse)
    ...
    var cl = $("#catList");
    if (cl) cl.addEventListener("click", onCatListClick);
  }

  // ---------- Categories ----------
  function loadCategories() {
    return apiFetch(apiUrl("bm_categories_list.php") + "?t=" + Date.now(), {
      cache: "no-store",
    })
      .then(parseJsonResponse)
      .then(function (data) {
      ...
      .catch(function (err) {
        logE("bm_categories_load_failed", {
  ...
  function onCatListClick(ev) {
    ...
    if (act === "cedit") {
      ...
      if (
        ...
        return;
      apiFetch(
        apiUrl("bm_categories_delete.php?id=" + encodeURIComponent(id)),
        { method: "POST" },
      )
        .then(parseJsonResponse)
        .then(() => {
  ...
  // ---------- Bookmarks ----------
  function loadList() {
    return apiFetch(apiUrl("bookmarks_list.php") + "?t=" + Date.now(), {
      cache: "no-store",
    })
      .then(parseJsonResponse)
      .then(function (data) {
  ...
  function currentCatFilter() {
    var v = ($("#bmCatFilter") && $("#bmCatFilter").value) || "";
    return v;
  }
  function currentSort() {
    var v = ($("#bmSort") && $("#bmSort").value) || "updated_desc";
    return v;
  }
  function renderList() {
    var list = $("#bmList"),
    ...
    var cat = currentCatFilter();
    var sort = currentSort();
    var items = BOOKS.slice();
    if (cat) {
      items = items.filter((b) => String(b.category_id || "") === String(cat));
    }
  ...
  function renderItem(it) {
    const fav =
      it.favicon && String(it.favicon).trim()
        ? resolveFaviconUrl(it.favicon)
        : apiUrl("favicon_proxy.php") +
          "?host=" +
          encodeURIComponent(it.host || "") +
          "&v=" +
          encodeURIComponent(it.updated || "");
