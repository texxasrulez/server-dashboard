(function () {
  ...
  function notifyAfterRender(section, pane) {
    window.__CONFIG_ACTIVE_SECTION__ = section;
    window.__CONFIG_PANE__ = pane || paneEl;
    var hooks = window.ConfigPageHooks || {};
    if (typeof hooks.afterRender !== "function") return;
    try {
      hooks.afterRender(section, pane || paneEl);
    } catch (error) {
      if (window.console) console.error("[config.page:afterRender]", error);
    }
  }
  ...
  function builtinEnhancers(section, pane) {
        ...
        if (!existingBtn) {
          ...
          if (hostInput) {
            ...
            btnDiag.addEventListener("click", function () {
              ...
              var transport =
                val(getByName("mail.mail_transport")) ||
    ...
    if (section.toLowerCase() === "history") {
      ...
      function makeCommand(what) {
        if (what === "history") {
          return "php bin/alerts-eval.php --probe=1";
        }
        return "php bin/alerts-eval.php";
      }

      function makeRunUrl(what) {
        try {
          var u = new URL("api/cron_run_admin.php", window.location.href);
          u.searchParams.set("kind", what);
          return u.toString();
        } catch (_) {
          return "api/cron_run_admin.php?kind=" + encodeURIComponent(what);
        }
      }
      ...
      function renderRow(label, last, everyMin, what) {
        ...
        refresh.addEventListener("click", function () {
          ...
          fetch(makeRunUrl(what), {
            ...
            .then(function (j) {
              if (!j || j.ok === false || !(j.ts || j.now)) {
                throw new Error((j && (j.error || j.message)) || "Run failed");
              }
              var ts = j.ts || j.now;
              var ms =
                typeof ts === "number"
                  ...
                  : Date.parse(ts);
              if (!isFinite(ms)) {
                throw new Error("Run completed but returned no timestamp");
              }
              ...
              window.toast?.success?.("Wrapper run completed");
            })
            .catch(function (err) {
              window.toast?.error?.(
                "Run failed: " + ((err && err.message) || err || "unknown"),
              );
            });
        });
  ...
  function sendTestEmail(to, opts) {
    ...
    fetch("api/mail_test.php", {
      ...
      .then(function (res) {
        ...
        return fetch("api/alerts_test.php", {
          method: "POST",
