...
(function () {
  ...
  if (window.__RATES_TABS_INIT__) return;
  window.__RATES_TABS_INIT__ = 1;
  ...
  function val(o, p, f) {
    try {
      return p.split(".").reduce((a, k) => a && a[k], o) ?? f;
    } catch (_) {
      return f;
    }
  }
  function ok(m) {
    window.toast && toast.success ? toast.success(m) : console.log(m);
  }
  function err(m) {
    window.toast && toast.error ? toast.error(m) : console.error(m);
  }
  ...
  function activeSection() {
    ...
    var elActive = root.querySelector(
      '.nav-tabs .active, .tabset .active, [role="tab"].active, [aria-selected="true"].nav-link',
    );
    ...
    var h = root.querySelector(
      ".tab-pane.active h2, .tab-pane.active h3, .config-pane.active h2, .config-pane.active h3",
    );
  ...
  function currentPane() {
    return (
      document.querySelector("#config-root .tab-pane.active") ||
      document.querySelector("#config-root .config-pane.active") ||
  ...
  function ensureServerTests(pane) {
    ...
    wrap.id = "server-tests-rate-card";
    wrap.style.marginTop = "12px";
    wrap.style.padding = "12px";
    var h = el("div", "h5");
    h.textContent = "Server Tests rate limit";
    var p = el("p");
    p.textContent = "Minimum gap between manual scans (ms). 0 disables.";
    var init = val(
      CUR,
      "config.server_tests.rate_limit_ms",
      val(CUR, "server_tests.rate_limit_ms", 250),
    );
    wrap.appendChild(h);
    wrap.appendChild(p);
    wrap.appendChild(
      makeRow("server_tests.rate_limit_ms", init, 50, "ms", function (v) {
        save({ server_tests: { rate_limit_ms: v } }, {});
      }),
    );
  ...
  function ensureAlerts(pane) {
    ...
    var wrap = el("div", "card");
    wrap.id = "alerts-cooldown-card";
    wrap.style.marginTop = "12px";
    wrap.style.padding = "12px";
    var h = el("div", "h5");
    h.textContent = "Alerts default cooldown";
    var p = el("p");
    p.textContent =
      "Fallback cooldown (minutes) when a rule omits it. Per-rule values still win.";
    var init = val(
      CUR,
      "config.alerts.cooldown_min",
      val(CUR, "alerts.cooldown_min", 5),
    );
    wrap.appendChild(h);
    wrap.appendChild(p);
    wrap.appendChild(
      makeRow("alerts.cooldown_min", init, 1, "min", function (v) {
        save({ alerts: { cooldown_min: v } }, {});
      }),
    );
    pane.appendChild(wrap);
  }
  ...
  function tick() {
    ...
    if (sec !== "server tests") {
      var a = document.getElementById("server-tests-rate-card");
      if (a && a.parentNode) a.parentNode.removeChild(a);
    }
    if (sec !== "alerts") {
      var b = document.getElementById("alerts-cooldown-card");
