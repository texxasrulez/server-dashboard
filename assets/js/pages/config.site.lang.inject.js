(function () {
  ...
  function isSitePane(n) {
    ...
    if ((n.getAttribute("data-pane") || "").toLowerCase() === "site")
      return true;
    return false;
  }
  function findSitePane() {
    var candidates = qa(
      '#pane-site, [data-pane="site"], .pane-site, .tab-pane.site',
    );
  ...
  function currentLocale() {
    try {
      if (
        ...
        window.__CONFIG_DATA__.i18n.locale
      )
        return String(window.__CONFIG_DATA__.i18n.locale);
    } catch (_) {}
  ...
  function ensureSelector() {
    ...
    fetch("api/i18n_languages.php", { credentials: "same-origin" })
      ...
      .then(function (j) {
        ...
        j.languages.forEach(function (it) {
          ...
          o.value = it.code;
          o.textContent = it.name || String(it.code || "").toUpperCase();
    ...
    // Save handler
    var btn = q("#siteLangSave", host),
      status = q("#siteLangStatus", host);
    if (btn) {
        ...
        fetch(url, {
          ...
          .then(function (j) {
            if (j && j.ok) {
              ...
              try {
                window.__CONFIG_DATA__ = window.__CONFIG_DATA__ || {};
                (window.__CONFIG_DATA__.i18n =
