(function () {
  ...
  function pane() {
    return (
      ...
      document.querySelector(".pane-site") ||
      document.querySelector(".tab-content, .content, main") ||
      document.body
    );
  }
  function hasCard(p) {
    return !!p.querySelector("#siteLangSelect");
  }
    ...
    fetch("api/i18n_languages.php", { credentials: "same-origin" })
      ...
      .then(function (j) {
        ...
        j.languages.forEach(function (it) {
          var o = el("option");
          o.value = it.code;
          o.textContent = it.name || (it.code || "").toUpperCase();
  ...
  window.addEventListener("hashchange", function () {
    if ((location.hash || "").toLowerCase().indexOf("site") >= 0) ensure();
  });
  document.addEventListener("click", function (e) {
    var a = e.target.closest('a[href*="#site"]');
    if (a) setTimeout(ensure, 0);
  });
})();
