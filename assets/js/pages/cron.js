(function () {
  ...
  var alertsEvalBin = root.dataset.alertsEvalBin || "bin/alerts-eval.php";
  var cronHeartbeatBin =
    root.dataset.cronHeartbeatBin || "bin/cron-heartbeat.php";
  ...
  function buildCommand(key) {
    if (key === "history") {
      return "php " + shellQuote(alertsEvalBin) + " --probe=1";
    }
    return "php " + shellQuote(alertsEvalBin);
  }
  ...
  function updateSnippets() {
    ...
    if (alertsCode)
      alertsCode.textContent = maskCommand(buildCommand("alerts"));
    if (histCode)
      histCode.textContent = maskCommand(buildCommand("history"));
  }
  ...
  function buildWrapperCommand(file, args) {
    var path = file === "cron-heartbeat.php" ? cronHeartbeatBin : alertsEvalBin;
    var cmd = "php " + shellQuote(path);
    if (args) cmd += " " + args;
    return cmd;
  }
  ...
  function buildWizardLines() {
    ...
    var alertsLine =
      ...
      " " +
      buildWrapperCommand("alerts-eval.php", "");
    var historyLine =
      scheduleFromMinutes(historyEvery?.value || 5) +
      " " +
      buildWrapperCommand("alerts-eval.php", "--probe=1");
    var jobLine =
      scheduleFromMinutes(jobEvery?.value || 5) +
      " " +
      buildWrapperCommand("cron-heartbeat.php", "--id=" + jobId);
  ...
  function ping(key, btn) {
    setBusy(btn, true);
    fetch("api/cron_run_admin.php?kind=" + encodeURIComponent(key), {
      method: "POST",
      ...
      .then(function (data) {
        if (!data || data.ok === false) {
          throw new Error((data && (data.error || data.message)) || "Run failed");
        }
        window.toast &&
          window.toast.success &&
