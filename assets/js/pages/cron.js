(function(){
  ...
  function chipClass(status){
    ...
    if (status !== 'ok' && status !== 'warn' && status !== 'fail') status = 'warn';
    return 'chip ' + status;
  }
  function buildUrl(key){
    ...
    try {
      ...
      if (!token) return raw;
      return raw + (raw.indexOf('?')>=0 ? '&' : '?') + 'token=' + encodeURIComponent(token);
    }
  }
  ...
  function renderCore(data){
    ...
    var defs = [
      {key:'alerts', label:'Alerts evaluator', desc:'Runs alert rules and notifications.', expect: expect.alerts_min},
      {key:'history', label:'History sampler', desc:'Writes probe samples for charts.', expect: expect.history_min}
    ];
    defs.forEach(function(def){
      var card = cronCore.querySelector('.cron-card[data-core="'+def.key+'"]');
      if (!card) return;
      ...
      card.innerHTML = [
        '<div class="card-head">',
          ...
          '<span class="'+chipClass(info.status)+'">'+escapeHtml((info.status||'warn').toUpperCase())+'</span>',
        '</div>',
        '<div class="meta-grid">',
          '<div><span class="muted">Last run</span><strong>'+escapeHtml(formatDate(info.last))+'</strong><span class="muted">'+escapeHtml(formatAgo(info.last))+'</span></div>',
          '<div><span class="muted">Next due</span><strong>'+escapeHtml(formatDate(info.next_due))+'</strong></div>',
          '<div><span class="muted">Expected every</span><strong>'+escapeHtml(def.expect ? def.expect + ' min' : '—')+'</strong></div>',
        '</div>',
        '<div class="card-actions">',
          '<button class="btn secondary" data-copy="'+def.key+'">Copy cURL</button>',
          '<button class="btn ghost" data-ping="'+def.key+'">Ping now</button>',
        '</div>'
      ].join('');
    });
  }
  ...
  function renderJobs(list){
    ...
    if (!list || !list.length){
      ...
      jobsList.innerHTML = '<div class="muted small">No custom jobs defined yet.</div>';
      return;
    }
      ...
      div.innerHTML = [
        '<div class="job-head">',
          '<div>',
            ...
            (job.line ? '<code class="job-line">'+escapeHtml(job.line)+'</code>' : ''),
          '</div>',
          '<span class="'+chipClass(job.status)+'">'+escapeHtml((job.status||'warn').toUpperCase())+'</span>',
        '</div>',
        '<div class="job-grid">',
          '<div><span class="muted">Last heartbeat</span><strong>'+escapeHtml(formatDate(job.last))+'</strong><span class="muted">'+escapeHtml(formatAgo(job.last))+'</span></div>',
          '<div><span class="muted">Heartbeat file</span><code>'+escapeHtml(job.heartbeat || '—')+'</code></div>',
          '<div><span class="muted">Next due</span><strong>'+escapeHtml(formatDate(job.next_due))+'</strong></div>',
          '<div><span class="muted">Stale after</span><strong>'+escapeHtml((job.stale_min||5)+' min')+'</strong></div>',
        '</div>',
        '<div class="job-actions">',
          (job.line ? '<button class="btn secondary" data-copy-line="'+job.id+'">Copy cron line</button>' : ''),
          (job.heartbeat ? '<button class="btn secondary" data-copy-heart="'+job.id+'">Copy heartbeat path</button>' : ''),
          '<button class="btn secondary" data-copy-hearturl="'+job.id+'">Copy API URL</button>',
        '</div>'
      ].join('');
  ...
  function checkHealthStatus(data){
    ...
    (data.jobs||[]).forEach(function(job){ apply(job.status); });
    if (level === lastSeverity) return;
    if (level === 'ok' && lastSeverity !== 'ok') {
      window.toast && window.toast.success && window.toast.success('Cron jobs recovered.');
    } else if (level === 'warn') {
      window.toast && window.toast.warn && window.toast.warn('Cron jobs are delayed.');
    } else if (level === 'fail') {
      window.toast && window.toast.error && window.toast.error('Cron jobs are failing or stale!');
    }
    lastSeverity = level;
  }
  ...
  function buildWizardLines(){
    ...
    var limitVal = Math.max(100, parseInt(limitInput?.value || '5000', 10) || 5000);
    var jobId = (jobIdInput?.value || 'custom_job').trim() || 'custom_job';
    var alertsLine = scheduleFromMinutes(alertsEvery?.value || 10) + ' curl -fsS "' + buildEvalUrl('alerts', limitVal) + '" >/dev/null 2>&1';
    var historyLine = scheduleFromMinutes(historyEvery?.value || 5) + ' curl -fsS "' + buildEvalUrl('history', limitVal) + '" >/dev/null 2>&1';
    var jobLine = scheduleFromMinutes(jobEvery?.value || 5) + ' curl -fsS "' + buildHeartbeatUrl(jobId) + '" >/dev/null 2>&1';
  ...
  function ping(key, btn){
    ...
    fetch(buildUrl(key), {credentials:'same-origin'})
      .then(function(r){ return r.text().then(function(t){ try{ return JSON.parse(t); } catch(_){ return null; } }); })
      .then(function(){ window.toast && window.toast.success && window.toast.success('Ping sent'); load(); })
      .catch(function(e){ window.toast && window.toast.error && window.toast.error('Ping failed: '+(e.message||e)); })
  ...
  function renderCrontab(items){
    ...
    items.forEach(function(line, idx){
      ...
      if (line.type === 'comment' || line.type === 'blank'){
        ...
        content = '<div><strong>Env</strong></div><div class="cron-command">'+escapeHtml(line.raw)+'</div>';
      } else {
        content =
          '<div><span class="muted">Schedule</span><strong>'+escapeHtml(line.schedule || '')+'</strong></div>' +
          '<div class="cron-command">'+escapeHtml(line.command || line.raw || '')+'</div>' +
          '<div class="line-actions"><button class="btn secondary" data-copy-cronline="'+idx+'">Copy</button></div>';
      }
  ...
  function loadCrontab(){
    ...
    fetch('api/cron_list.php?_=' + Date.now(), {credentials:'same-origin'})
      ...
      .then(function(data){
        if (!data || !data.ok){
          crontabList.innerHTML = '<div class="muted small" style="padding:.75rem;">'+escapeHtml(data && data.error ? data.error : 'Unable to read crontab')+'</div>';
          if (crontabNote) crontabNote.textContent = 'Failed to read crontab (exit '+((data && data.exit)!=null?data.exit:'?')+').';
          return;
        }
      ...
      .catch(function(err){
        crontabList.innerHTML = '<div class="muted small" style="padding:.75rem;">'+escapeHtml(err.message||err)+'</div>';
  ...
  function load(){
    ...
    fetch('api/cron_health.php?_=' + Date.now(), {credentials:'same-origin'})
      ...
      .then(function(data){
        ...
        if (statusNote){
          statusNote.textContent = 'Updated ' + formatDate(data.now || Math.floor(Date.now()/1000));
        }
        checkHealthStatus(data);
      })
      .catch(function(err){
        console.error(err);
        window.toast && window.toast.error && window.toast.error('Cron health load failed: ' + (err.message||err));
      })
      .finally(function(){
  ...
  root.addEventListener('click', function(ev){
    ...
    var heartUrlBtn = target.closest && target.closest('[data-copy-hearturl]');
    if (heartUrlBtn){
      ev.preventDefault();
      var job3 = jobCache.get(heartUrlBtn.getAttribute('data-copy-hearturl'));
      var id = job3 ? (job3.id || '') : (heartUrlBtn.getAttribute('data-copy-hearturl') || '');
    ...
    if (cronline){
      ...
      var node = crontabList && crontabList.querySelectorAll('.crontab-line')[idx];
  ...
  if (tokenToggle && tokenField){
    tokenToggle.addEventListener('click', function(){
      ...
      var nowType = tokenField.getAttribute('type') === 'password' ? 'text' : 'password';
  ...
  if (wizard){
    ['wizardAlertsEvery','wizardHistoryEvery','wizardLimit','wizardJobId','wizardJobEvery'].forEach(function(id){
      var el = document.getElementById(id);
      if (el) el.addEventListener('input', function(){ updateWizardOutputs(); });
    });
    updateWizardOutputs();
  }
  updateSnippets();
