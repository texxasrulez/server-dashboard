(function () {
  ...
  function toggleRowDetails(tr) {
    ...
    var nameCell = tr.cells && tr.cells[0];
    if (!nameCell) return;
    ...
    fetch("database.php?action=tables&db=" + encodeURIComponent(db))
      ...
      .then(function (j) {
        if (!j || !j.ok) {
          td.innerHTML = '<div class="error">Failed to load tables</div>';
          return;
        }
        var html = [
          '<div class="db-details"><table class="table compact"><thead><tr><th>Table</th><th>Rows</th><th>Data</th><th>Index</th><th>Updated</th></tr></thead><tbody>',
        ];
        (j.tables || []).forEach(function (t) {
          html.push(
            '<tr class="table-row" data-db="' +
              escapeHtml(j.db) +
              '" data-table="' +
              escapeHtml(t.name) +
              '">',
          ...
          html.push(
            '<td class="tbl-name"><button class="linklike">' +
              escapeHtml(t.name) +
              "</button></td>",
          );
          html.push(
          ...
          html.push(
            "<td>" + (t.update_time ? escapeHtml(t.update_time) : "") + "</td>",
          );
          html.push("</tr>");
        });
  ...
  function openModal(db, table, payload) {
    ...
    if (!el) {
      ...
      on($(".modal-close", el), "click", function () {
        el.setAttribute("hidden", "");
      });
    ...
    cols.forEach(function (c) {
      out.push("<th>" + escapeHtml(c) + "</th>");
    });
    out.push("</tr></thead><tbody>");
    rows.forEach(function (r) {
      out.push("<tr>");
      cols.forEach(function (c) {
        out.push("<td>" + escapeHtml(r[c]) + "</td>");
      });
      out.push("</tr>");
    });
  ...
  function handleBodyClicks(ev) {
    ...
    if (trow) {
      ...
      fetch(
        "database.php?action=preview&db=" +
        ...
        .then(function (j) {
          if (j && j.ok) {
            ...
            if (window.toast)
              window.toast.error((j && j.error) || "Preview failed");
          }
        })
        .catch(function (err) {
          if (window.toast)
            window.toast.error((err && err.message) || "Network error");
        });
      return;
    }
