(function () {
  ...
  function load() {
    fetch(scanUrl(), { cache: "no-store" })
      ...
      .catch(function (err) {
        ...
        tb.innerHTML =
          '<tr><td colspan="9" class="muted">Error: ' +
  ...
  function renderServer(srv) {
    ...
    var txt =
      "Server: " + srv.host + ":" + srv.port + "  •  Version: " + srv.version;
    if (srv.uptime) txt += "  •  Uptime: " + fmtUptime(srv.uptime);
    if (srv.threads_connected != null)
      txt +=
        "  •  Threads: " +
        srv.threads_connected +
        " (" +
        (srv.threads_running || 0) +
        " running)";
    el.textContent = txt;
  }
  ...
  function renderTable(rows) {
    ...
    if (!rows.length) {
      tb.innerHTML =
        '<tr><td colspan="9" class="muted">No databases found.</td></tr>';
      return;
    }
    var html = rows
      .map(function (r) {
        var sys = r.system ? '<span class="badge">system</span>' : "";
        var last = r.last_update
          ? r.last_update.replace("T", " ").replace(".000Z", "")
          : "";
        return (
          "<tr>" +
          "<td>" +
