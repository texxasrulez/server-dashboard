(function () {
  ...
  function on(el, ev, fn) {
    el && el.addEventListener(ev, fn, false);
  }
  ...
  function close() {
    var m = ensureModal();
    m.hidden = true;
    var c = $(".modal-content", m);
    if (c) c.innerHTML = "";
    var t = $(".modal-title", m);
    if (t) t.textContent = "";
    $("#modalTools", m).innerHTML = "";
  }

  function setTitle(t) {
    var m = ensureModal();
    var el = $(".modal-title", m);
    if (el) el.textContent = t || "";
  }
  function setTools(nodes) {
    var m = ensureModal();
    var wrap = $("#modalTools", m);
    wrap.innerHTML = "";
  ...
  function stripChrome(html) {
    ...
    var doc = tpl.content;
    var main =
      doc.querySelector("main.container, main, .content, .card, body") || doc;
    var out = create("div", "embedded");
    if (main) {
      var kill = doc.querySelectorAll(
        "header, nav, .app-header, footer, .usermenu, .theme-picker",
      );
  ...
  function ensureAuditControls(ctx) {
    ...
    var tools =
      ctx.querySelector(".audit-toolbar") || create("div", "audit-toolbar");
    if (!tools.parentNode) {
      var head = ctx.querySelector(".modal-tools") || ctx;
      head.appendChild(tools);
    }
    var b1 = create("button", "btn");
    b1.id = "aa-scan";
    b1.textContent = "Scan now";
    var b2 = create("button", "btn secondary");
    b2.id = "aa-open";
    b2.textContent = "Open JSON";
    var b3 = create("button", "btn secondary");
    b3.id = "aa-copy";
    b3.textContent = "Copy report";
    tools.innerHTML = "";
        ...
        try {
          if (t.id === "aa-scan") {
            ...
            if (typeof toast === "function")
              window.toast.confirmation("Assets scan complete");
            ...
            if (t.id === "aa-copy") {
              (async () => {
                ...
                if (typeof toast === "function")
                  window.toast.confirmation("Report copied");
              })();
          ...
          if (typeof toast === "function") window.toast.error("Action failed");
        }
      },
      false,
    );
  ...
  async function openMetrics(url) {
    ...
    try {
      ...
      try {
        ...
        data = { error: "Non‑JSON response", body: txt.slice(0, 400) };
      }
      var wrap = create("div", "json-wrap");
      var pre = create("pre", "json");
      pre.textContent = JSON.stringify(data, null, 2);
      wrap.appendChild(pre);
      setContent(wrap);
    } catch (e) {
      var err = create("div");
      err.textContent = "Load failed: " + ((e && e.message) || e);
      setContent(err);
    }
  }
  ...
  document.addEventListener("click", function (ev) {
    ...
    setTitle(a.textContent.trim() || "Details");
    open();
    fetch(href, { credentials: "same-origin" })
      ...
      .catch(function (e) {
        var d = create("div");
        d.textContent = "Load failed: " + ((e && e.message) || e);
        setContent(d);
      });
  });
})();
