(function(){
  ...
  function drawSpark(cv, series, okBad, showBaseline, crossX, selX1, selX2, colors){
    ...
    while (startIndex < series.length) {
      ...
      for (var j=startIndex+1; j<=endIndex; j++) ctx.lineTo(x(j), y(series[j]));
      ctx.stroke();
  ...
  function downsampleSeries(series, maxPoints){
    ...
    var dsTimes=[], dsLat=[], dsOks=[], dsCodes=[];
  ...
  function buildExtras(it){
    ...
    if (typeof it.value !== 'undefined' && it.value !== null){
      parts.push('value: <strong>'+escapeHtml(String(it.value))+'</strong>');
    }
    if (it.http_code){
  ...
  function ensureMetaChips(){
    ...
    if (ex) return ex;
    var w = document.createElement('div', 'card'); w.id=id; w.className='row gap-sm wrap'; w.style.marginLeft='auto';
    w.innerHTML =
  ...
  function updateMetaChips(){
    ensureMetaChips();
    var zb = $('#zoomBadge'), cb = $('#zoomClearBtn'), lp = $('#lastProbeChip'), ri = $('#rangeInfoChip');
    if (ZOOM && zb && cb){
    ...
    if (ri){
      ...
      ri.textContent = 'Interval ≈ ' + interval + 's · Coverage ' + formatDurationShort(Math.min(target, coverage)) + '/' + formatDurationShort(target);
      ri.hidden = false;
    }
    if (lp){
      if (LAST_PROBE_TS){
  ...
  function buildCardActions(id){
    ...
    var rules = Array.isArray(alertMeta.rule_ids) ? alertMeta.rule_ids.filter(Boolean) : [];
    if (rules.length){
  ...
  function renderCards(items, since){
    ...
    if (CURRENT_SERVICE !== '__all') {
      ids = ids.filter(function(id){ return id === CURRENT_SERVICE; });
    }
    ...
    ids.sort().forEach(function(id){
      ...
      var oksRaw = arr.map(function(x){ return (x.status==='up'||x.ok===1||x.ok===true)?1:0; });
      var codes = arr.map(function(x){ return x.http_code||''; });
      var ds = downsampleSeries({times: times, lat: lat, oks: oksRaw.map(Boolean), codes: codes}, DOWNSAMPLE_MAX);
      ...
      var uptimePct = arr.length ? Math.round(arr.reduce(function(acc,it){ return acc + ((it.status==='up'||it.ok===1||it.ok===true)?1:0); },0) / arr.length * 100) : null;

      var card = document.createElement('div'); card.className='card';
      if (usingFallback) card.style.opacity = .75;
      var stateClass = null;
      var lastOk = !!(last && (last.status === 'up' || last.ok === true || last.ok === 1));
      ...
      cv._series = { times: timesDS, lat: latDS, oks: oksDS, codes: codesDS, name: name };

      var palette = { ok: '#22a559', warn: '#f0b429', fail: '#d95140' };
      drawSpark(cv, latDS, oksDS, showBaseline, undefined, undefined, undefined, palette);
      ...
      on(cv, 'mousemove', throttle(function(ev){
        ...
        var when = new Date((cv._series.times[i]||0)*1000).toLocaleString();
        var v = latDS[i]|0, ok = !!cv._series.oks[i], code = cv._series.codes[i] || '';
        ...
        drawSpark(cv, latDS, oksDS, showBaseline, p.px, selPx1, selPx2, palette);
        showTip(html, ev.clientX, ev.clientY);
      }, 16));
      on(cv, 'mouseleave', function(){ hideTip(); drawSpark(cv, latDS, oksDS, showBaseline, undefined, undefined, undefined, palette); });
      ...
      on(cv, 'mousedown', function(ev){
        ...
        drawSpark(cv, latDS, oksDS, showBaseline, null, selPx1, selPx2, palette);
      });
      ...
      on(csvBtn, 'click', function(){
        ...
        for (var i=0;i<lat.length;i++){
          var t=times[i], iso = new Date(t*1000).toISOString();
  ...
  function handleCardAction(ev){
    ...
    if (!btn) return;
    ev.preventDefault();
    var rules = (btn.dataset.rules||'').split(',').map(function(s){ return s.trim(); }).filter(Boolean);
    if (!rules.length){
      window.toast && window.toast.warn && window.toast.warn('No alert rules for this service.');
      return;
    }
    var svcId = btn.dataset.service || '';
    var svcName = SERVICE_MAP[svcId] || (SERVICE_META[svcId] && SERVICE_META[svcId].name) || svcId || 'service';
    ...
    btn.disabled = true;
    silenceRuleIds(rules, mins).then(function(){
  ...
  function fetchText(url, opts){ return fetch(url, Object.assign({ credentials:'include', cache:'no-store' }, opts||{})).then(r=>r.text().then(t=>({ok:r.ok, status:r.status, text:t}))); }
  function fetchJSON(url, opts){ return fetchText(url, opts).then(({ok, status, text})=>{ var j=parseLooseJSON(text); if(!ok && j && j.error) throw new Error(j.error+' (HTTP '+status+')'); if(!ok) throw new Error('HTTP '+status); return j||{}; }); }
  function silenceRuleIds(ids, minutes){
    if (!ids || !ids.length) return Promise.reject(new Error('No alert rules available'));
  ...
  function loadAutoInterval(){
    return fetchJSON('api/autoprobe_get.php').then(function(j){
      if (j && j.interval){
  ...
  function applyAutoprobeUI(cfg){
    ...
    if (input) input.value = Math.max(5, parseInt(cfg.interval || AUTO_INTERVAL_SEC || 60, 10) || 60);
    if (status){
      if (cfg.enabled){
        status.textContent = 'Running every ' + Math.max(5, parseInt(cfg.interval||60,10)) + 's';
      } else {
  ...
  function renderAlerts(items){
    ...
    if (!rows.length){
      ...
      trEmpty.innerHTML = '<td colspan="6" class="muted">No alert events in this range.</td>';
    ...
    rows.forEach(function(it){
      ...
      tr.innerHTML = '<td><div class="time-cell"><span class="time-rel">'+escapeHtml(rel)+'</span><span class="time-abs">'+escapeHtml(abs)+'</span></div></td>'
                   + '<td>'+escapeHtml(svc)+'</td>'
                   + '<td>'+escapeHtml(it.alert_name||'')+'</td>'
                   + '<td>'+escapeHtml(cond)+'</td>'
                   + '<td>'+extras+'</td>'
                   + '<td>'+severityBadge(it.severity||'warn')+'</td>';
      tb.appendChild(tr);
    });
  }
  ...
  function load(range){
    ...
    var alertsLimit = Math.max(200, Math.round(Math.max(1000, probesLimit / 4)));

    var probesUrl = root.dataset.exportProbes + '&limit=' + probesLimit + '&start=' + since + perServiceParam + '&_=' + Date.now();
    var alertsUrl = root.dataset.exportAlerts + '&limit=' + alertsLimit + '&start=' + since + '&_=' + Date.now();
    ...
    return Promise.all([ fetchJSON(probesUrl), fetchJSON(alertsUrl), fetchJSON(cronUrl), servicesListPromise ]).then(function(res){
      ...
      var svcItems = (svcPayload && Array.isArray(svcPayload.items)) ? svcPayload.items : [];
      ...
      if (diag) {
        var coverage = formatDurationShort(Math.min(rangeDuration(CURRENT_RANGE), LAST_COVERAGE_SEC||0));
        var target = formatDurationShort(rangeDuration(CURRENT_RANGE));
        diag.textContent = 'Probes: '+(_CACHE.probes.length|0)+' · Alerts: '+(_CACHE.alerts.length|0)+' · Interval ≈ '+Math.max(5, AUTO_INTERVAL_SEC||60)+'s · Coverage '+coverage+'/'+target+' · Updated '+new Date().toLocaleTimeString();
      }
  ...
  function refreshServiceSelect(){
    ...
    if (!needsRebuild) {
      var values = Array.prototype.map.call(sel.options, function(opt){ return opt.value; }).join(',');
      var expected = ['__all'].concat(unique).join(',');
      if (values !== expected) needsRebuild = true;
    }
    if (needsRebuild) {
      sel.innerHTML = '';
      var optAll = document.createElement('option'); optAll.value='__all'; optAll.textContent='All services';
  ...
  function wire(){
    ...
    window.addEventListener('autoprobe:config', function(ev){ applyAutoprobeUI(ev.detail||{}); });
    if (AUTOPROBE_UI.btn){
      on(AUTOPROBE_UI.btn, 'click', function(){
        ...
        var intervalVal = Math.max(5, parseInt((AUTOPROBE_UI.input && AUTOPROBE_UI.input.value) || AUTO_INTERVAL_SEC || 60, 10) || 60);
        ...
        window.AutoProbe.set({
          ...
          window.toast && window.toast.success && window.toast.success('Autoprobe updated');
        }).catch(function(err){
          toastError('Autoprobe update failed: ' + (err && err.message ? err.message : err || 'unknown'));
        }).finally(function(){
    ...
    on(toolbar, 'click', function(ev){
      ...
      if (t.id==='resetHistoryBtn' || act==='reset-history' || txt==='reset history'){
        ...
        setBusy(t, true);
        fetchJSON('api/history_rotate.php', { method:'POST' }).then(function(j){
    ...
    function doExport(which){
      ...
      fetchJSON(url + '&limit=' + limit + '&start=' + since + extra).then(function(j){
