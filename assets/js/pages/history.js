(function () {
  ...
  function renderAlerts(items) {
    ...
    rows.forEach(function (it) {
      ...
      tr.innerHTML =
        ...
        '"><div class="time-cell"><span class="time-rel">' +
        escapeHtml(rel) +
        ...
        "<td>" +
        (it.incident_id
          ? '<a href="incident.php?id=' +
            encodeURIComponent(it.incident_id) +
            '">' +
        ...
        "</td>" +
        '<td data-value="' +
        escapeHtml(String(Number(it.value) || 0)) +
        '">' +
        extras +
        "</td>" +
        "<td>" +
  ...
  function renderIncidents(items) {
    var tb = $("#incidentsTable tbody");
    if (!tb) return;
    tb.innerHTML = "";
    var rows = Array.isArray(items) ? items.slice(0, 20) : [];
    if (!rows.length) {
      var trEmpty = document.createElement("tr");
      trEmpty.innerHTML =
        '<td colspan="6" class="muted">No correlated incidents in this range.</td>';
      tb.appendChild(trEmpty);
      return;
    }
    rows.forEach(function (it) {
      ...
      tr.innerHTML =
        '<td data-value="' +
        escapeHtml(String(it.last_ts || 0)) +
        '">' +
        escapeHtml(fmtDT(it.last_ts || 0)) +
        "</td>" +
        '<td><a href="incident.php?id=' +
        encodeURIComponent(it.id || "") +
        '">' +
        escapeHtml(it.title || "Incident") +
        "</a></td>" +
        "<td>" +
        escapeHtml(it.root_service_name || it.root_service_id || "") +
        "</td>" +
        "<td>" +
        escapeHtml(it.status || "open") +
        "</td>" +
        '<td data-value="' +
        escapeHtml(String(it.event_count || 0)) +
        '">' +
        escapeHtml(String(it.event_count || 0)) +
        "</td>" +
        "<td>" +
  ...
  function load(range) {
    ...
    var alertsUrl =
      ...
      "&_=" +
      Date.now();
    var incidentsUrl = (root.dataset.incidents || "api/incidents.php") + "?_=" + Date.now();
    ...
    return Promise.all([
      ...
      .then(function (res) {
        ...
        _CACHE.incidents = res[2] && res[2].items ? res[2].items : [];
        SERVICE_MAP = {};
        ...
        renderIncidents(_CACHE.incidents);
        if (diag) {
