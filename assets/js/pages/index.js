(function () {
  ...
  function showDashTip(html, x, y) {
    ...
    var nx = Math.min(x + 14, window.innerWidth - r.width - 8),
      ny = Math.min(y + 12, window.innerHeight - r.height - 8);
    t.style.left = nx + "px";
    t.style.top = ny + "px";
  }
  function hideDashTip() {
    var t = window.dashTipElem;
    if (t) t.hidden = true;
  }
  ...
  function bindSparkTooltip(canvas, series, opts) {
    ...
    var now = Date.now() / 1000,
      n = series.length,
      step = 60;
    var times = Array.from({ length: n }, function (_, i) {
      return now - (n - 1 - i) * step;
    });
  ...
  async function fetchMetrics() {
    const res = await fetch("api/metrics.php", { credentials: "same-origin" });
    return res.json();
  }
  ...
  function renderChartsOverlay(m) {
    ...
    if (b)
      b.textContent = `Load avg: ${load[0].toFixed(3)}, ${load[1].toFixed(3)}, ${load[2].toFixed(3)}`;
  ...
  function renderMetricsCharts(m) {
    ...
    const loadSeries = Array.from(
      { length: 60 },
      () => load[0] + (Math.random() - 0.5) * 0.05,
    );

    window.__sparkLoad = loadSeries;
    const memSeries = Array.from(
      { length: 60 },
      () => memUsed + (Math.random() - 0.5) * 0.03,
    );
    ...
    (function () {
      var n = 60,
        step = 60,
        now = Date.now() / 1000;
      window.__sparkTimes = Array.from(
        { length: n },
        (_, i) => now - (n - 1 - i) * step,
      );
    })();
    line(
      $("#chartLoad"),
      loadSeries.map((v) => Math.max(0, v)),
    );
    bindSparkTooltip($("#chartLoad"), loadSeries, {
      name: "CPU load",
      fmt: function (v) {
        return v.toFixed(2);
      },
    ...
    line(
      $("#chartMem"),
      memSeries.map((v) => Math.max(0, v)),
    );
    bindSparkTooltip($("#chartMem"), memSeries, {
      name: "Memory used",
      fmt: function (v) {
        return Math.round(Math.max(0, Math.min(1, v)) * 100) + "%";
      },
    });
    line($("#chartNet"), netSeries);
    bindSparkTooltip($("#chartNet"), netSeries, {
      name: "Net activity",
      fmt: function (v) {
  ...
  function renderServices(m) {
    ...
    items.forEach((s) => {
      ...
      const card = el(
        ...
        s.http_code
          ? el("div", { class: "http-note" }, "HTTP " + s.http_code)
          : document.createTextNode(""),
      );
      wrap.appendChild(card);
    });
    const upEl = $("#svcUpCount");
    if (upEl) upEl.textContent = upCount;
    const totEl = $("#svcTotal");
  ...
  function renderProcs(m) {
    ...
    const items = [
      ...
      {
        title: "Memory Used",
        val: Math.round(usedPct),
        sub: `${fmtGB(used)} / ${fmtGB(total)}`,
      },
      { title: "Memory Free", val: Math.round(freePct), sub: fmtGB(avail) },
      {
        title: "Buffers",
        val: Math.round(bufPct),
        sub: fmtGB(mem.buffers || 0),
      },
      {
        title: "Cache",
        val: Math.round(cachePct),
        sub: fmtGB(mem.cached || 0),
      },
    ];
    grid.innerHTML = "";
    items.forEach((p) => {
      const bar = el("div", { class: "vbar" });
      bar.style.setProperty("--val", p.val);
      grid.appendChild(
        el(
          "div",
          { class: "proc" },
  ...
  function renderDisks(m) {
    const list = m.disks || [];
    const root = list.find((d) => d.mount === "/") ||
      list[0] || { total: 0, used: 0, free: 0, pct_used: 0, mount: "/" };
    const usedPct = Math.round(
      root.pct_used ?? (root.total ? (root.used / root.total) * 100 : 0),
    );
    ...
    tiles.forEach((t) => {
      const dev = el(
        ...
        el(
          "div",
          { class: "hdd-slot" },
          el("div", { class: "progress", style: `--val:${t.pct}` }),
        ),
      );
      const right = el(
        "div",
  ...
  async function refresh() {
    const m = await fetch("api/metrics.php", {
      credentials: "same-origin",
    }).then((r) => r.json());
    renderChartsOverlay(m);
