(function () {
  ...
  function ensureTip() {
    var t = document.getElementById("sparkTip");
    if (t) return t;
    t = document.createElement("div", "card");
    t.id = "sparkTip";
  ...
  function bind(canvas, series, name, fmt) {
    ...
    function idx(ev) {
      ...
      var i = Math.round(
        (series.length - 1) * (rect.width ? x / rect.width : 0),
      );
    ...
    ["mouseleave", "mousedown", "blur"].forEach(function (ev) {
      canvas.addEventListener(ev, hideTip, false);
    });
  }
  function ready(fn) {
  ...
  ready(function () {
    ...
    var t = setInterval(function () {
      ...
      if (
        ...
        bind(load, L, "CPU load", function (v) {
          return (v || 0).toFixed(2);
        });
        bind(mem, M, "Memory used", function (v) {
          v = Math.max(0, Math.min(1, v));
          return Math.round(v * 100) + "%";
        });
        bind(net, N, "Net activity", function (v) {
