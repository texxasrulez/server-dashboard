(function () {
  ...
  function badge(ok) {
    return `<span class="chip ${ok ? "ok" : "bad"}">${ok ? "OK" : "FAIL"}</span>`;
  }

  function renderPHP(d) {
    cards.php.innerHTML =
      row("Version", d.php.version) + row("SAPI", d.php.sapi);
  }
  ...
  function renderOPC(d) {
    if (!d.opcache) {
      cards.opcache.innerHTML =
        '<div class="muted">OPcache not available</div>';
      return;
    }
    var m = d.opcache.memory || {};
    var s = d.opcache.stats || {};
    var mem =
      m.used_memory != null && m.free_memory != null
        ? `${(m.used_memory / 1048576).toFixed(1)} MB used / ${(m.free_memory / 1048576).toFixed(1)} MB free`
        : "n/a";
    var rate =
      s.opcache_hit_rate != null ? s.opcache_hit_rate.toFixed(2) + "%" : "n/a";
    cards.opcache.innerHTML =
      row("Enabled", badge(!!d.opcache.enabled)) +
      row("Memory", mem) +
      row(
        "Cached Scripts",
        s.num_cached_scripts != null ? s.num_cached_scripts : "n/a",
      ) +
      row("Hit rate", rate);
  }
  function renderFS(d) {
    ...
    html += row(
      "Disk free",
      disk.free != null ? (disk.free / 1073741824).toFixed(2) + " GB" : "n/a",
    );
    html += row(
      "Disk total",
      disk.total != null ? (disk.total / 1073741824).toFixed(2) + " GB" : "n/a",
    );
  ...
  function copyJSON() {
    try {
      ...
      navigator.clipboard.writeText(txt).then(function () {
        toast && toast.success && toast.success("JSON copied");
      });
    } catch (e) {
  ...
  function exportJSON() {
    var blob = new Blob(
      [JSON.stringify(window.__SERVER_TEST_LAST || {}, null, 2)],
