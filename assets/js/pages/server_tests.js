(function () {
  ...
  function ensureInfoModal() {
    ...
    el.addEventListener("click", function (ev) {
      if (ev.target === el) el.setAttribute("hidden", "");
    });
    document.addEventListener("keydown", function (ev) {
      if (ev.key === "Escape") el.setAttribute("hidden", "");
    });
    return el;
  }
  ...
  function getInfoContent(name) {
    ...
    var blocks = [];
    function h(t) {
      return '<h3 style="margin:0 0 8px 0;">' + t + "</h3>";
    }
    function p(t) {
    ...
    switch (n) {
      case "NTP sync":
        ...
        blocks.push(
          p(
            "If everything looks good but the scanner still warns, your system may use journald only; that’s fine.",
          ),
        );
        break;
      ...
      default:
        ...
        blocks.push(
          p(
            "Start with service status, recent logs, and config. If you tell me the stack used here, I can add targeted steps.",
          ),
        );
    }
    ...
    blocks.push(
      p(
        "These are starting points, not one-click magic. Systems differ; adjust to your stack.",
      ),
    );
    return blocks.join("");
  }
  ...
  function chipClassForStatus(st) {
    ...
    if (
      ...
      return "ok";
    if (st === "warn" || st === "warning" || st === "medium" || st === "notice")
      return "warn";
  ...
  function cfg(path, defVal) {
    try {
      var obj =
        window.__CONFIG_DATA__ && window.__CONFIG_DATA__.server_tests
          ? window.__CONFIG_DATA__.server_tests
          : {};
      var key = path
        .replace(/^server_tests\./, "")
        .replace(/^history\./, "")
        .replace(/^alerts\./, "");
      var v = obj[key];
  ...
  function ensureServicesAndRun() {
    ...
    var cfgTargets =
      ...
      window.__CONFIG_DATA__.server_tests &&
      Array.isArray(window.__CONFIG_DATA__.server_tests.service_targets)
        ? window.__CONFIG_DATA__.server_tests.service_targets
        : null;
    ...
    if (!arr.length) {
      var t = prompt(
        "Enter services as host:port|label, comma-separated",
        "127.0.0.1:80|Nginx, 127.0.0.1:3306|MySQL",
      );
      if (!t) {
        window.toast &&
          window.toast.info &&
          window.toast.info("No services provided");
    ...
    return fetch(url, {
      ...
      .then(function (r) {
        if (!r.ok) {
          ...
          if (r.status === 429)
            throw new Error("Please wait a moment (rate limited)");
      ...
      .catch(function (e) {
        ...
        if (window.toast && window.toast.error)
          window.toast.error(String(e && e.message ? e.message : e));
        throw e;
      });
  }
  ...
  function tooltipFor(row) {
    ...
    if (n === "TLS cert expiry") {
      var w = cfg("server_tests.tls_warn_days", 21),
        f = cfg("server_tests.tls_fail_days", 7);
      return "Warn < " + w + " days; Fail < " + f + " days";
    }
    if (n === "Inodes free") {
      var ok = cfg("server_tests.inode_ok_free_pct", 15),
        warn = cfg("server_tests.inode_warn_free_pct", 7);
      return "OK ≥ " + ok + "%, Warn ≥ " + warn + "% free";
    }
    if (n === "Security updates") {
      var wr = cfg("server_tests.updates_warn_count", 1),
        fl = cfg("server_tests.updates_fail_count", 10);
  ...
  function renderResults(block) {
    try {
      ...
      block.results.forEach(function (r) {
        ...
        var fixBtn =
          cls !== "ok"
            ? '<button type="button" class="btn secondary copy-fix" data-name="' +
              esc(r.name) +
              '"' +
        ...
        tr.innerHTML =
          ...
          '<code style="white-space:normal;word-break:break-word;overflow-wrap:anywhere;">' +
          esc(r.details || "") +
          "</code>" +
          (fixBtn || "") +
          "</div>" +
      ...
      console && console.error && console.error(e);
      if (window.toast && window.toast.error)
        window.toast.error("Render failed");
    }
  }

  function setReportCard(score) {
    var badge = $("#reportCard");
    if (!badge) return;
    var meta = (function (score) {
      if (score >= 0.9) return { grade: "A", color: "ok" };
      if (score >= 0.8) return { grade: "B", color: "ok" };
      if (score >= 0.7) return { grade: "C", color: "warn" };
      if (score >= 0.6) return { grade: "D", color: "warn" };
  ...
  function drawLineChart(canvas, series, key) {
    ...
    var px = function (x) {
      return ((W - 40) * (x - minX)) / (maxX - minX + 1e-9) + 30;
    };
    var py = function (y) {
      return (H - 30) * (1 - (y - minY) / (maxY - minY + 1e-9)) + 10;
    };
    ctx.lineWidth = 2;
    ctx.beginPath();
    ...
    fetchHistory(DAYS)
      .then(function (res) {
        ...
        tbody.innerHTML = recent
          .map(function (r) {
            var bad = (r.items || []).filter(function (x) {
              return (x.status || "warn") !== "ok";
            });
            return (
              "<tr><td>" +
        ...
        // export and range
        var hj = document.getElementById("hExportJson");
        if (hj)
          hj.addEventListener("click", function () {
            download("history.json", JSON.stringify({ points: pts }, null, 2));
          });
        var hc = document.getElementById("hExportCsv");
        if (hc)
        ...
        Array.prototype.slice
          .call(document.querySelectorAll(".h-range"))
          .forEach(function (b) {
            b.addEventListener("click", function () {
              ...
              fetchHistory(dd)
                .then(function (res2) {
                  ...
                  drawLineChart(
                    document.getElementById("hDisk"),
                    p2,
                    "disk_free_pct",
                  );
                  drawLineChart(
                    document.getElementById("hUpd"),
                    p2,
                    "security_updates",
                  );
                  drawLineChart(
                    document.getElementById("hLoad"),
                    p2,
                    "load_1m",
                  );
                  var tbody2 = document.getElementById("hRecent");
                  var recent2 = res2.recent || [];
                  tbody2.innerHTML = recent2
                    .map(function (r) {
                      var bad = (r.items || []).filter(function (x) {
                        return (x.status || "warn") !== "ok";
                      });
    ...
    if (s)
      q &&
        s.addEventListener("click", function () {
          run("security").catch(function () {});
        });
    if (f)
      f.addEventListener("click", function () {
        run("filesystem").catch(function () {});
      });
    if (v)
      v.addEventListener("click", function () {
        ensureServicesAndRun().catch(function () {});
      });
    if (p)
      p.addEventListener("click", function () {
        run("performance").catch(function () {});
      });
    if (h)
    ...
    if (bj)
      bj.addEventListener("click", function () {
        if (!lastResults || !lastResults.results) {
          window.toast &&
            window.toast.info &&
            window.toast.info("Run a test first");
          return;
        }
