...
(function () {
  ...
  if (!subhead) {
    ...
    if (headerRow) {
      ...
      (host || document.body).insertBefore(
        subhead,
  ...
  const cards = qa(".card").filter((c) => {
    ...
    return (
      !!t && /php|opcache|extensions|filesystem|environment|network/.test(t)
    );
  });
  ...
  cards.forEach((card) => {
    ...
    else if (t.includes("environment") || t === "env")
      card.classList.add("st-env");
  });
  ...
  if (fs) {
    ...
    const blocks = qa(
      ":scope > .kv, :scope > .section, :scope > .row, :scope > div",
      body,
    ).filter(
      (el) =>
        el !== body &&
