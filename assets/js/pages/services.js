(function () {
  ...
  function decorateRow(row, svc) {
    ...
    if (svc.id) {
      ...
      detail.className = "chip neutral small";
      detail.textContent = t("services_page.details", "Details");
      detail.href = "service_detail.php?id=" + encodeURIComponent(svc.id);
      linkRow.appendChild(detail);
    }
    const incidentMeta = svc.incident_meta || {};
    if (incidentMeta.incident_id) {
      const incident = document.createElement("a");
      incident.className = "chip warn small";
      incident.textContent = t("services_page.incident", "Incident");
      incident.href =
        "incident.php?id=" + encodeURIComponent(incidentMeta.incident_id);
      linkRow.appendChild(incident);
    }
