(function () {
  ...
  function newCtrl() {
    abortFetch();
    try {
      FETCH_CTRL = new AbortController();
    } catch (_) {
      FETCH_CTRL = null;
    }
    return FETCH_CTRL;
  ...
  async function ensureEnabledSet() {
    ...
    try {
      const r = await fetch(
        LIST_API,
        { cache: "no-store" },
        (typeof LIST_API, { cache: "no-store" } === "object"
          ? LIST_API,
            { cache: "no-store" }
          : {})
      );
  ...
  function pillEl(card) {
    return card.querySelector('.pill,[data-role="svc-pill"]');
  }
  function latEl(card) {
    return card.querySelector('.svc-latency,[data-role="svc-latency"]');
  }
  ...
  function extractStatus(o) {
    ...
    if (s) {
      if (/^(up|ok|online|open|alive|running|passing|pass|success|good)$/.test(s))
    ...
    if (typeof o?.alive === "boolean")
      return { state: o.alive ? "up" : "down", latency: lat };
  ...
  function getCards() {
    let nodes = Array.from(
      document.querySelectorAll(
        '#services .service-card, #services .svc-card, #services [data-svc-id], #services [data-svc-name], #services [data-role="svc-card"]'
      )
    );
    ...
    pills.forEach((p) => {
      const host = p.closest(
        "[data-svc-id], [data-svc-name], .service-card, .svc-card, .card, li, div"
      );
  ...
  function setPill(card, state) {
    const pill = pillEl(card);
    if (!pill) return;
    const cur = pill.classList.contains("up")
      ? "up"
      : pill.classList.contains("down")
        ? "down"
  ...
  async function fetchAndApply() {
    try {
      const r = await fetch(
        STATUS_API,
        { cache: "no-store" },
        (typeof STATUS_API, { cache: "no-store" } === "object"
          ? STATUS_API,
            { cache: "no-store" }
          : {})
      );
