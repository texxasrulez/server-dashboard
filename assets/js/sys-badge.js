(function () {
  ...
  function label(up, total, cron) {
    ...
    if (total) {
      if (up === total) {
        kind = "ok";
        text = `All systems operational  ${up}/${total} up`;
      } else if (up > 0) {
        kind = "warn";
        text = `Degraded  ${up}/${total} up`;
      } else {
        kind = "danger";
        text = `Major outage  ${up}/${total} up`;
      }
    }
  ...
  function idOf(x) {
    return norm(x.id || x.name || x.title);
  }
  ...
  async function refresh() {
    try {
      ...
      try {
        ...
        const results = Array.isArray(status && status.results)
          ? status.results
          : Array.isArray(status && status.items)
            ? status.items
            : [];
        ...
        try {
          ...
          const results = Array.isArray(s && s.results)
            ? s.results
            : Array.isArray(s && s.items)
              ? s.items
              : [];
          const resMap = new Map(results.map((x) => [idOf(x), x]));
          for (const [id, svc] of enabledMap) {
