(function () {
  "use strict";
  if (window.__TOAST_POS_AUTO__) return;
  window.__TOAST_POS_AUTO__ = true;
  var POS_KEY = "ui.toast_position";

  function readPos() {
    try {
      return localStorage.getItem(POS_KEY) || "bottom-center";
    } catch (e) {
  ...
  function isToastLike(el) {
    ...
    if (
      ...
      return true;
    return /(toast|notify|notyf|izi|snackbar|snack|alertify|toastr|toastify)/.test(
      cls,
    );
  }
  ...
  function applyAll() {
    ...
    var nodes = document.querySelectorAll(
      '[role="alert"],[aria-live],.toast,.toast-container,.toastify,.iziToast,.iziToast-wrapper,.notyf,.snackbar,.alertify-notifier,#toast-root,.toast-root,[data-toast-root]',
    );
        ...
        if (m.addedNodes && m.addedNodes.length) {
          for (var j = 0; j < m.addedNodes.length; j++) {
            ...
            if (
              isToastLike(n) ||
              (n.querySelector &&
                n.querySelector('.toast, [role="alert"], [aria-live]'))
            ) {
              changed = true;
              break;
            }
