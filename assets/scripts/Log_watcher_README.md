...
The installed service should run as `root` so it can read root-owned logs under `/var/log`. The scripts then apply the configured `--owner` to the copied files in the destination directory so the dashboard can read them without making the source logs world-readable.
...
- If some current vhost logs appear only when you run `sudo /usr/local/bin/copy_log_once.sh`, your watcher is likely running unprivileged. Reinstall or update the service so `log-watcher.service` runs as `root`.
