...
/* Local page header scrolls with the rest of the backups page */
...
<section class="grid" style="display:flex; gap:16px; margin-bottom:16px; flex-wrap:wrap;">
  ...
  <script>
    ...
    function renderVerification(data){
      const target = document.getElementById('backupVerifyStatus');
      const history = document.getElementById('backupVerifyHistory');
      if (!target || !history) return;
      const latest = data && data.latest ? data.latest : null;
      if (!latest || !latest.ts) {
        target.textContent = 'No restore verification recorded yet.';
        history.innerHTML = '';
        return;
      }
      target.textContent = 'Last verification: ' + latest.ts + ' · ' + (latest.method || 'integrity-verified') + ' · ' + (latest.result || 'unknown');
      const checks = Array.isArray(latest.checks) ? latest.checks : [];
      history.innerHTML = checks.map(check => {
        const label = check && check.label ? check.label : 'artifact';
        const result = check && check.result ? check.result : 'unknown';
        const reason = check && check.reason ? check.reason : '';
        return '<div class="chip ' + (result === 'pass' ? 'ok' : (result === 'warn' ? 'warn' : 'down')) + '">' +
          label + ': ' + result + (reason ? ' · ' + reason : '') + '</div>';
      }).join('');
    }

    function loadVerification(){
      fetch('api/backup_verify.php?_=' + Date.now(), { credentials: 'same-origin' })
        .then(r => r.json())
        .then(data => {
          if (data && data.ok) renderVerification(data);
        })
        .catch(() => {});
    }

    function runVerification(){
      const btn = document.getElementById('runBackupVerify');
      if (btn) btn.disabled = true;
      fetch('api/backup_verify.php', {
        method: 'POST',
        ...
        .then(r => r.json())
        .then(data => {
          if (!data || !data.ok) throw new Error((data && data.error) || 'Verification failed');
          renderVerification({ latest: data.result });
          window.toast?.success?.('Backup verification finished');
        })
        .catch(err => {
          window.toast?.error?.(err.message || 'Verification failed');
        })
        .finally(() => {
    ...
    function buildSupportBundle(){
      const btn = document.getElementById('buildSupportBundle');
      const target = document.getElementById('supportBundleStatus');
      if (btn) btn.disabled = true;
      if (target) target.textContent = 'Building support bundle…';
      fetch('api/support_bundle.php', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'X-CSRF-Token': CSRF_TOKEN }
      })
        .then(r => r.json())
        .then(data => {
          if (!data || !data.ok || !data.bundle) throw new Error((data && data.error) || 'Bundle generation failed');
          if (target) {
            target.innerHTML = 'Support bundle ready: <a href="' + data.bundle.download + '">' + data.bundle.filename + '</a>';
          }
          window.toast?.success?.('Support bundle generated');
        })
        .catch(err => {
          if (target) target.textContent = err.message || 'Bundle generation failed';
          window.toast?.error?.(err.message || 'Bundle generation failed');
        })
        .finally(() => {
          if (btn) btn.disabled = false;
        });
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      loadCrontab();
      loadVerification();
      document.getElementById('runBackupVerify')?.addEventListener('click', runVerification);
      document.getElementById('buildSupportBundle')?.addEventListener('click', buildSupportBundle);
    });

  </script>

  <div class="card collapsible-card" data-collapse-key="verification-support" style="margin-bottom:24px;">
    <div class="card-header">
      <div>
        <div class="card-title">Restore Verification & Support</div>
        <div class="card-sub">Conservative artifact verification plus one-click redacted troubleshooting bundles.</div>
      </div>
      <button type="button" class="collapsible-toggle" data-collapse-toggle aria-expanded="true" title="Collapse/Expand">▾</button>
    </div>
    <div class="collapsible-body">
      <div style="display:flex; flex-wrap:wrap; gap:16px; padding:10px 4px;">
        <div style="flex:1 1 320px; min-width:280px;">
          <h3 style="margin-top:0; font-size:14px;">Backup Restore Verification</h3>
          <div class="muted small" id="backupVerifyStatus">Loading verification status…</div>
          <div id="backupVerifyHistory" class="pill-row" style="margin-top:10px;"></div>
          <div class="row gap-sm" style="margin-top:10px;">
            <button type="button" class="btn-backup" id="runBackupVerify">Run Verification</button>
          </div>
        </div>
        <div style="flex:1 1 320px; min-width:280px;">
          <h3 style="margin-top:0; font-size:14px;">Troubleshooting Bundle</h3>
          <div class="muted small">Includes redacted config summary, services snapshot, recent incidents, audit events, backup verification history, and selected log tails.</div>
          <div class="row gap-sm" style="margin-top:10px;">
            <button type="button" class="btn-backup" id="buildSupportBundle">Build Support Bundle</button>
          </div>
          <div class="muted small" id="supportBundleStatus" style="margin-top:10px;"></div>
        </div>
      </div>
    </div>
  </div>
