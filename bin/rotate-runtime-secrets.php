...
function set_path(array &$arr, string $path, $value): void
{
    ...
    foreach ($parts as $p) {
        ...
        if (!array_key_exists($p, $cur) || !is_array($cur[$p])) {
            $cur[$p] = $cur[$p] ?? [];
        }
        $cur = & $cur[$p];
    }
...
foreach ($optionalCronPaths as $path) {
    ...
    if ($exists) {
        set_path($cfg, $path, $newCronToken);
    }
}
...
if (!is_dir(dirname($dataCron))) {
    @mkdir(dirname($dataCron), 0775, true);
}
@file_put_contents($dataCron, $newCronToken . PHP_EOL);
if (is_file($stateCron)) {
    @file_put_contents($stateCron, $newCronToken . PHP_EOL);
}

@chmod($cfgPath, 0640);
@chmod($dataCron, 0640);
if (is_file($stateCron)) {
    @chmod($stateCron, 0640);
}
