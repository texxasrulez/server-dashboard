...
$REQUIRE_ADMIN = true;
$bookmarkSourceDefault = \App\Config::get('bookmarks.default_source', 'local');
$roundDavConfigured = RoundDavBookmarks::isConfigured();
require_once __DIR__ . '/includes/head.php';
?>
<main class="wrap" id="bookmarks-root"
  data-api-base="<?= h(project_url('/api/')) ?>"
  data-default-source="<?= h($bookmarkSourceDefault === 'rounddav' ? 'rounddav' : 'local') ?>"
  data-rounddav-configured="<?= $roundDavConfigured ? '1' : '0' ?>">
 <div class="card">
  <div class="card">
    <div class="left">
...
<a class="btn ghost"
   ...
   data-i18n-attr="title"><span data-i18n="bookmarks_page.add_to_browser"><?= h(__('bookmarks_page.add_to_browser', 'Add to Browser')) ?></span></a>
    ...
    <div class="right">
      <div class="controls">
        <label class="inline">
          ...
          <select id="bmSource">
            ...
            <option value="rounddav" <?= $roundDavConfigured ? '' : 'disabled' ?>>RoundDAV</option>
          </select>
        </label>
        <label class="inline">
          <span data-category-label data-i18n="bookmarks_page.category">Category</span>
      ...
      <div id="bmSourceNote" class="muted small"></div>
    </div>
   </div>
  </header>
  ...
  <section class="grid-2">
    ...
    <section class="card add-form">
      <form id="bmForm">
        <div class="row">
          ...
          <label><span data-category-label data-i18n="bookmarks_page.category">Category</span>
    ...
    <!-- Right: Add Category -->
    <section class="card cats local-only" id="bmCategoryCreateCard">
      ...
      <div class="modal-grid">
        <section class="card add-form">
          ...
          <form id="bmFormModal" class="row">
            ...
            <label><span data-category-label data-i18n="bookmarks_page.category">Category</span> <select id="bmCategory"></select></label>
        ...
        <section class="card cats local-only" id="bmCategoryManageCard">
