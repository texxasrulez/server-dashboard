...
if (
    ...
    try {
        $incomingSettings = is_array($payload["settings"] ?? null)
            ? $payload["settings"]
            : [];
        ...
        try {
            ...
            if (isset($saved) && is_array($saved)) {
                if (isset($saved["alerts"]["cron_token"])) {
                    $sources[] = (string) $saved["alerts"]["cron_token"];
                }
                if (isset($saved["security"]["cron_token"])) {
                    $sources[] = (string) $saved["security"]["cron_token"];
                }
                if (isset($saved["site"]["cron_token"])) {
                    $sources[] = (string) $saved["site"]["cron_token"];
                }
                if (isset($saved["cron"]["token"])) {
                    $sources[] = (string) $saved["cron"]["token"];
                }
                if (isset($saved["api"]["cron_token"])) {
                    $sources[] = (string) $saved["api"]["cron_token"];
                }
                if (isset($saved["history"]["token"])) {
                    $sources[] = (string) $saved["history"]["token"];
                }
            }
            // Also consider raw payload if present
            if (isset($payload["settings"]) && is_array($payload["settings"])) {
                $ps = $payload["settings"];
                if (isset($ps["alerts"]["cron_token"])) {
                    $sources[] = (string) $ps["alerts"]["cron_token"];
                }
                if (isset($ps["security"]["cron_token"])) {
                    $sources[] = (string) $ps["security"]["cron_token"];
                }
                if (isset($ps["site"]["cron_token"])) {
                    $sources[] = (string) $ps["site"]["cron_token"];
                }
                if (isset($ps["cron"]["token"])) {
                    $sources[] = (string) $ps["cron"]["token"];
                }
                if (isset($ps["api"]["cron_token"])) {
                    $sources[] = (string) $ps["api"]["cron_token"];
                }
                if (isset($ps["history"]["token"])) {
                    $sources[] = (string) $ps["history"]["token"];
                }
                if (isset($payload["CRON_TOKEN"])) {
                    $sources[] = (string) $payload["CRON_TOKEN"];
                }
            }
        ...
        AuditLog::record(
            ...
            true,
            [
                "sections" => array_keys($incomingSettings),
                "top_level_count" => count($incomingSettings),
                "changed" => Redaction::sanitizeForPersisted($incomingSettings),
            ],
            "config save ok",
            "config",
        );
        echo json_encode(["ok" => true, "config" => $saved]);
        exit();
    } catch (Exception $e) {
        AuditLog::record(
            "config.save",
...
$PAGE_JS = [
    "assets/js/pages/config.page.js",
    "assets/js/pages/config.security.js",
];
...
if (!is_file($schema_path)) {
    ...
    die(
        "Missing schema at {$schema_path}. Expected /config/schema.php under project root."
    );
}
...
<?php // Standard footer (footer text, toast auto-init, scripts, etc.)
