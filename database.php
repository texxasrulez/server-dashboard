...
if (($_GET['action'] ?? '') === 'scan') {
    ...
    $mysqli = @mysqli_init();
    if (!$mysqli) {
        echo json_encode(['ok' => false,'error' => 'mysqli_init failed']);
        exit;
    }
    @mysqli_options($mysqli, MYSQLI_OPT_CONNECT_TIMEOUT, 5);
    $link = @$mysqli->real_connect($host, $user, $pass, null, $port);
    if (!$link) {
        echo json_encode(['ok' => false, 'error' => 'Connect failed', 'errno' => $mysqli->connect_errno, 'err' => $mysqli->connect_error]);
        exit;
    }
    ...
    if ($res = $mysqli->query("SHOW GLOBAL STATUS WHERE Variable_name IN ('Uptime','Threads_connected','Threads_running')")) {
        while ($row = $res->fetch_assoc()) {
            $statusRows[$row['Variable_name']] = (int)$row['Value'];
        }
        $res->free();
    }
...
if (($_GET['action'] ?? '') === 'tables') {
    ...
    $db = (string)($_GET['db'] ?? '');
    if ($db === '') {
        echo json_encode(['ok' => false,'error' => 'missing db']);
        exit;
    }
    ...
    if ($res = @$mysqli->query("SHOW TABLE STATUS FROM `{$dbIdent}`")) {
        ...
        echo json_encode(['ok' => false,'error' => 'Query failed','errno' => $mysqli->errno,'err' => $mysqli->error]);
        exit;
    }
    echo json_encode(['ok' => true,'db' => $db,'tables' => $tables]);
    exit;
}
...
if (($_GET['action'] ?? '') === 'preview') {
    ...
    if ($db === '' || $table === '') {
        echo json_encode(['ok' => false,'error' => 'missing args']);
        exit;
    }
    ...
    if (!@$mysqli->select_db($db)) {
        echo json_encode(['ok' => false,'error' => 'select_db failed']);
        exit;
    }
    ...
    $res->free();
    echo json_encode(['ok' => true,'db' => $db,'table' => $table,'columns' => $cols,'rows' => $rows,'count' => count($rows),'limit' => $limit]);
    exit;
}
