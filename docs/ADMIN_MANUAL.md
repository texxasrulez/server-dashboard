...
This manual captures everything needed to install, operate, and extend the Server Dashboard. It combines the scattered quick notes, how‑tos, and README fragments into one canonical reference for administrators and power users.
...
2. Ensure the following directories are writable by the web user:
   - `data/` — configuration exports and backup files.
   - `state/` — runtime JSON streams (`backup_status.json`, services status, alerts).
3. Create and maintain `config/local.json` through the Config UI (see §4).
...
Navigate to **Config** to edit `config/local.json`. Each tab corresponds to a top-level key:

- **Site**: name, base URL, timezone, theme. These values drive the header brand and time formatting.
- **Features**: toggle all top-level feature pages. Unchecked features are hidden from the header and blocked on direct access until re-enabled.
...
- **Security**: admin emails, CSRF secret, API tokens, and IP allow list.
- **Alerts**: email/webhook recipients, mute presets, service defaults.
- **History/Logs**: retention limits, log watcher settings.
- **Email**: header indicator accounts (stored as a JSON string).

Saving pushes updates to both `config/local.json` and `data/security_config.json` so legacy scripts stay in sync. Use the **“Send Test Email”** button to validate SMTP once credentials are in place.
...
- Monthly uptime reporting now uses time-weighted up/down intervals with coverage percentage instead of simple sample counting.
...
- `Open HTML` opens the generated report inside a modal; CSV remains a direct export.
...
3. **Crontab helper**: fill in intervals, job IDs, and it generates:
   ...
   - Remote heartbeat URLs (`api/cron_heartbeat.php?id=job`).
   - Ready‑made cron entries (`crontab -e`) without token parameters.
...
```
GET https://<host>/api/cron_heartbeat.php?id=JOB_ID
```
...
- **Environment Doctor** on `diag.php` groups checks into `PASS`, `WARN`, and `FAIL` for PHP/runtime/config/file health.
- **Assets Audit** opens from `diag.php` and reports missing asset references, orphaned page assets, inventory totals, and largest assets.
- **Admin Audit** opens from `diag.php` and shows recent security and diagnostic actions from the dashboard log files.
...
- **Cron helpers**: the generated lines on `cron.php` no longer include cron tokens for alerts, history, or heartbeat jobs.
...
| Symptom                | Resolution                                                                                                                                                              |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `403` from cron `curl` | Confirm you are using the current token-free helper URL from `cron.php` and that any IP allow lists or server auth layers allow the request.                            |
| History cards empty    | Run `api/alerts_eval.php?probe=1` manually and check `state/services_status_history.jsonl` for corruption.                                                              |
...
Use these after changes or upgrades:
...
Run browser smoke after page or JS changes:

```bash
bash bin/browser-smoke.sh
```

Expected browser smoke passes:
...
Expected browser smoke skips:

- no Chrome/Chromium installed
- local TCP listener unavailable in the host environment

Those skip messages indicate environment limits, not dashboard failure.
...
## 13. Appendix — Useful Paths & Files

| File/Dir                                | Purpose                                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------- |
| `config/local.json`                     | Primary persisted configuration edited via UI.                                |
| `data/security_config.json`             | Legacy compat file for mail and security-related settings.                    |
| `data/logs/security.log`                | Security events and related admin audit entries.                              |
...
| `state/logs/diag_audit.log`             | Manual diagnostics and server-test audit events.                              |
