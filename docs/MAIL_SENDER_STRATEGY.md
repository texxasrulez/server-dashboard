...
- If PHP or a shell script sends mail without an explicit envelope sender, the MTA can fall back to the runtime account such as `www-data@domain.net` or `root@host`.
...
- Set both `mail.mail_from` and `mail.mail_envelope_from` to a real hosted mailbox such as `admin@domain.net`.
