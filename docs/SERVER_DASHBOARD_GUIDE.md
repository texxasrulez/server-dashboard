...
4. **Bootstrap first admin (CLI):**
   ...
   ```
5. **Open** `https://your-host/` (or your chosen base path) and log in as `admin` with the printed password, then rotate it immediately in `Users`.
...
> Protect with HTTPS. Consider Basic Auth or IP allowlists on the dashboard base path if you do not already have app auth enabled.
...
- Cron example (runs every hour):
  ```cron
  5 * * * * curl -fsS https://your-host/api/alerts_run.php >/dev/null
  ```
...
- If your PHP CLI environment is compatible with your web runtime:
  ```cron
  5 * * * * php /var/www/your-site/api/alerts_run.php >/dev/null 2>&1
  ```
