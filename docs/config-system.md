...
Use the Config → Site → “Create backup” action (or `api/config_backup.php`) to snapshot `config/local.json` into `config/backups/config-YYYYmmdd-HHMMSS.json`. The alerts admin page can also create alert-rule snapshots in the same location as `config/backups/alerts-YYYYmmdd-HHMMSS.json`, and restore the latest alerts snapshot back into `data/alerts.json`. Retention + download/prune all operate on that directory.
