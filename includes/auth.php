...
function user_find($username)
{
    ...
    foreach ($data['users'] as $u) {
        if (isset($u['username']) && strtolower($u['username']) === strtolower((string)$username)) {
            return $u;
        }
    }
    return null;
...
function auth_login($username, $password)
{
    ...
    if (!$u) {
        auth_login_rate_register_failure($username, $ip);
        return false;
    }
...
{
    $cu = current_user();
    if (strtolower($cu['username'] ?? '') === strtolower($username)) {
