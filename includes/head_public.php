...
<head>
  ...
  <?php
    ...
    $ASSETS_PREFIX = isset($ASSETS_PREFIX) ? trim($ASSETS_PREFIX) : 'assets';
$assetsBase = rtrim($ASSETS_PREFIX, '/');
$theme_href_public = $assetsBase . '/css/themes/' . basename($THEME) . '.css?v=' . h(BUILD);
$page_css_href = null;
if (!empty($PAGE_CSS)) {
    if (strpos($PAGE_CSS, 'assets/') === 0) {
        $page_css_href = $ASSETS_PREFIX . substr($PAGE_CSS, strlen('assets'));
    } else {
        $page_css_href = $PAGE_CSS;
    }
}
?>
  <link rel="stylesheet" href="<?= h($assetsBase) ?>/css/theme/core.css?v=<?= h(BUILD) ?>" />
  <link rel="stylesheet" href="<?= h($theme_href_public) ?>" />
  <?php if (!empty($page_css_href)) : ?>
    <link rel="stylesheet" href="<?= h($page_css_href) ?>" />
  <?php endif; ?>
  <script defer src="<?= h($assetsBase) ?>/js/app.js?v=<?= h(BUILD) ?>"></script>
</head>
