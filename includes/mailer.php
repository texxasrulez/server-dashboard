...
if (!class_exists('Mailer')) {
    class Mailer
    {
        public static function transport()
        {
            ...
            $t = strtolower(trim($t));
            if ($t === 'phpmail' || $t === 'sendmail' || $t === 'smtp') {
                return $t;
            }
            ...
            if ($te === 'phpmail' || $te === 'sendmail' || $te === 'smtp') {
                return $te;
            }
            return 'phpmail';
        }
        ...
        {
            $from = self::cfg('mail.mail_from', mailer_env('MAIL_FROM', ''));
            $from = self::normalizeMailboxHeader($from);
            if ($from !== '') {
                return $from;
            }

            $envelope = self::envelopeFrom();
            if ($envelope !== '') {
                return $envelope;
            }

            return self::defaultEnvelopeFrom();
        }

        public static function replyTo()
        {
            $reply = self::cfg('mail.mail_replyto', mailer_env('MAIL_REPLYTO', ''));
            return self::normalizeMailboxHeader($reply);
        }

        public static function envelopeFrom($candidateHeaderFrom = '')
        {
            $envelope = self::cfg('mail.mail_envelope_from', mailer_env('MAIL_ENVELOPE_FROM', ''));
            $envelope = self::normalizeEnvelopeEmail($envelope);
            if ($envelope !== '') {
                return $envelope;
            }

            $fallback = self::normalizeEnvelopeEmail($candidateHeaderFrom);
            if ($fallback !== '') {
                return $fallback;
            }
        ...
        public static function send($to, $subject, $body, $opts = array())
        {
            ...
            if (!count($toList)) {
                return array('ok' => false, 'error' => 'no recipients');
            }

            $subject = self::encodeHeader(self::stripNewlines($subject ? $subject : '(no subject)'));
            $from    = self::normalizeMailboxHeader(isset($opts['from']) ? $opts['from'] : self::from());
            if ($from === '') {
                $from = self::from();
            }
            $reply   = self::normalizeMailboxHeader(isset($opts['reply_to']) ? $opts['reply_to'] : self::replyTo());
            $envelopeFrom = self::envelopeFrom($from);

            $normalizedRecipients = array();
            for ($i = 0; $i < count($toList); $i++) {
                $normalized = self::normalizeRecipient($toList[$i]);
                if ($normalized === '') {
                    return array('ok' => false, 'error' => 'invalid recipient: ' . self::stripNewlines($toList[$i]));
                }
            ...
            if ($t === 'phpmail') {
                ...
                $ok = @mail(implode(', ', $normalizedRecipients), $subject, $body, $headerStr, $params);
                $result['ok'] = $ok ? true : false;
                if (!$ok) {
                    $result['error'] = 'phpmail send failed';
                }
            } elseif ($t === 'sendmail') {
                ...
                if (!$h) {
                    ...
                    fwrite($h, 'To: ' . implode(', ', $normalizedRecipients) . "\r\n");
                    ...
                    if ($code === 0) {
                        ...
                        $result['error'] = 'sendmail exit '.$code;
                    }
                }
            } else { // smtp
                $cfg = self::smtpConfig();
                ...
                if (!$fp) {
                    $result['error'] = 'connect failed: '.$errno.' '.$errstr;
                    mailer_log($result);
                    return $result;
                }
                ...
                if ($cfg['secure'] === 'tls') {
                    ...
                    if ($code !== 220) {
                        fclose($fp);
                        $result['error'] = 'starttls failed '.$code;
                        mailer_log($result);
                        return $result;
                    }
                    if (!stream_socket_enable_crypto($fp, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
                        fclose($fp);
                        $result['error'] = 'tls upgrade failed';
                        mailer_log($result);
                        return $result;
                    }
                    fwrite($fp, "EHLO ".$ehloHost."\r\n");
                    ...
                    if ($code !== 250) {
                        fclose($fp);
                        $result['error'] = 'post-tls ehlo failed '.$code;
                        mailer_log($result);
                        return $result;
                    }
                }
                ...
                if ($cfg['user'] !== '' && $cfg['pass'] !== '') {
                    ...
                    if ($code !== 334) {
                        fclose($fp);
                        $result['error'] = 'auth login not accepted '.$code;
                        mailer_log($result);
                        return $result;
                    }
                    ...
                    if ($code !== 334) {
                        fclose($fp);
                        $result['error'] = 'username rejected '.$code;
                        mailer_log($result);
                        return $result;
                    }
                    ...
                    if ($code !== 235) {
                        fclose($fp);
                        $result['error'] = 'password rejected '.$code;
                        mailer_log($result);
                        return $result;
                    }
                }
                ...
                if ($code !== 250) {
                    fclose($fp);
                    $result['error'] = 'MAIL FROM failed '.$code;
                    mailer_log($result);
                    return $result;
                }

                for ($i = 0; $i < count($normalizedRecipients); $i++) {
                    $rcpt = self::extractEmail($normalizedRecipients[$i]);
                    fwrite($fp, "RCPT TO:<".$rcpt.">\r\n");
                    $code = self::smtpReadCode($fp);
                    if ($code !== 250 && $code !== 251) {
                        fclose($fp);
                        $result['error'] = 'RCPT TO failed '.$code;
                        mailer_log($result);
                        return $result;
                    }
                }
                ...
                if ($code !== 354) {
                    fclose($fp);
                    $result['error'] = 'DATA not accepted '.$code;
                    mailer_log($result);
                    return $result;
                }

                $msg = self::buildRfc822($normalizedRecipients, $from, $subject, $headers, $body);
                ...
                if ($code !== 250) {
                    fclose($fp);
                    $result['error'] = 'message not accepted '.$code;
                    mailer_log($result);
                    return $result;
                }
        ...
        private static function smtpConfig()
        {
            ...
            $cfg = array(
              ...
              'secure'  => $secure,
              'timeout' => (int)self::cfg('mail.smtp_timeout', mailer_env('SMTP_TIMEOUT', 12)),
            );
            // normalize port
            if ($cfg['port'] === '') {
                $cfg['port'] = 0;
        ...
        public static function normalizeEOL($s)
        {
            return str_replace(array("\r\n","\r"), "\n", (string)$s);
        }
        public static function encodeHeader($s)
        {
        ...
        private static function normalizeRecipient($s)
        {
            $raw = self::stripNewlines(trim((string)$s));
            if ($raw === '') {
                return '';
            }
            $email = self::normalizeEnvelopeEmail($raw);
            if ($email === '') {
                return '';
            }
            $header = self::normalizeMailboxHeader($raw);
            return $header !== '' ? $header : $email;
        }
        private static function normalizeMailboxHeader($s)
        {
            $raw = self::stripNewlines(trim((string)$s));
            if ($raw === '') {
                return '';
        ...
        private static function normalizeEnvelopeEmail($s)
        {
            $email = self::extractEmail($s);
            $email = self::stripNewlines(trim((string)$email));
            if ($email === '') {
                return '';
            }
