...
final class Config {
  ...
  public static function delete(string $path): bool {
    ...
    if (!self::removePath($current, $segments)) return false;
    $defaults = require self::$path . '/defaults.php';
...
if ($val === '' || $val === null) {
  ...
  private static function mergeLegacySecurity(array $local): array {
    ...
    if (!is_file($secFile)) return $local;
    $raw = @file_get_contents($secFile);
    if ($raw === false) return $local;
    $legacy = json_decode($raw, true);
    if (!is_array($legacy) || !$legacy) return $local;
    ...
    foreach ($map as $legacyKey => $path) {
      ...
      if (self::pathHasValue($local, $path)) continue;
    ...
    try {
      ...
      foreach ($map as $key => $value) {
        if ($key === 'smtp_pass' && ($value === null || $value === '')) {
          if (isset($apply[$key]) && $apply[$key] !== '') continue;
        }
        if ($value === null) {
          unset($apply[$key]);
          continue;
  ...
  private static function emailsToString($value): string {
    if (!is_array($value)) return is_string($value) ? trim($value) : '';
    $clean = [];
  ...
  private static function normalizeArray($value): array {
    ...
    if (is_string($value)) return self::splitEmails($value);
    return [];
  }
  ...
  private static function removePath(array &$tree, array $segments): bool {
    ...
    if ($key === null || !array_key_exists($key, $tree)) return false;
    if ($segments === []) {
      unset($tree[$key]);
      return true;
    }
    if (!is_array($tree[$key])) return false;
