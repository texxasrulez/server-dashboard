...
class ServerDiag
{
    ...
    public static function check_tls_cert($host)
    {
        ...
        if (!$host) {
            return ['name' => 'TLS cert expiry','status' => 'warn','details' => 'no host'];
        }
        ...
        if (!$fp) {
            return ['name' => 'TLS cert expiry','status' => 'warn','details' => 'unreachable'];
        }
        $params = stream_context_get_params($fp);
        @fclose($fp);
        if (empty($params['options']['ssl']['peer_certificate'])) {
            return ['name' => 'TLS cert expiry','status' => 'warn','details' => 'no cert'];
        }
        $cert = $params['options']['ssl']['peer_certificate'];
        $info = openssl_x509_parse($cert);
        $to = $info['validTo_time_t'] ?? null;
        if (!$to) {
            return ['name' => 'TLS cert expiry','status' => 'warn','details' => 'invalid cert'];
        }
