...
final class Speedtest
{
    ...
    public static function buildPayload(array $filters): array
    {
        ...
        $summary['next_scheduled_test'] = self::estimateNextDue(self::settings(), self::loadMeta(), time());
        return [
            'filters' => $filters,
            'rows' => $rows,
    ...
    private static function readToolVersion(string $binary, string $backend): array
    {
        ...
        if ($backend === 'ookla' && $binaryName === 'speedtest-cli') {
            return [
                'version' => '',
                'mode' => 'speedtest-cli',
            ];
        ...
        if (!$res['ok']) {
            $fallbackMode = self::detectBinaryModeFromHelp($binary, $backend);
            return [
                'version' => '',
                'mode' => $fallbackMode,
            ];
        ...
        if ($mode === $backend && $backend === 'ookla') {
            $mode = self::detectBinaryModeFromHelp($binary, $backend);
        }
        return [
            'version' => $line,
            'mode' => $mode,
    ...
    private static function detectBinaryModeFromHelp(string $binary, string $backend): string
    {
        if ($backend !== 'ookla') {
            return $backend;
        }

        $res = self::execCommand(escapeshellarg($binary) . ' --help', 4);
        $output = strtolower(trim($res['stdout'] . "\n" . $res['stderr']));
        if ($output === '') {
            return $backend;
        }

        if (
            str_contains($output, '--accept-license')
            || str_contains($output, '--accept-gdpr')
            || str_contains($output, '--format=')
        ) {
            return 'ookla';
        }

        if (
            str_contains($output, '--no-download')
            || str_contains($output, '--no-upload')
            || str_contains($output, '--csv-delimiter')
            || str_contains($output, '[--json]')
        ) {
            return 'speedtest-cli';
        }
    ...
    private static function execCommand(string $command, int $timeoutSeconds): array
    {
        ...
        $code = self::normalizeProcessExitCode($code, $lastStatus);
        if ($code !== 0) {
    ...
    private static function normalizeProcessExitCode(int $code, ?array $lastStatus): int
    {
        if ($code !== -1) {
            return $code;
        }
        ...
        $exitCode = $lastStatus['exitcode'] ?? null;
    ...
    private static function appendHistoryRow(array $row, array $settings): array
    {
        ...
        if (function_exists('dashboard_sync_default_metadata')) {
            dashboard_sync_default_metadata($path, $metadataSource);
        }
