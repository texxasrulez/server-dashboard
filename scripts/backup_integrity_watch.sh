...
EMAIL_FROM="${BACKUP_ALERT_EMAIL_FROM:-${MAIL_FROM:-server-dashboard@localhost.invalid}}"
EMAIL_ENVELOPE_FROM="${BACKUP_ALERT_EMAIL_ENVELOPE_FROM:-${MAIL_ENVELOPE_FROM:-$EMAIL_FROM}}"
...
if ! {
  ...
  echo "Sender: $EMAIL_ENVELOPE_FROM"
  echo "MIME-Version: 1.0"
  echo "Content-Type: text/html; charset=UTF-8"
  echo
  printf "%s" "$HTML"
} | "$SENDMAIL" -t -i -f "$EMAIL_ENVELOPE_FROM"; then
