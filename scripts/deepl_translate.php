...
if (!$key) {
    fwrite(STDERR, "DEEPL_API_KEY not set\n");
    exit(3);
}
...
if (!is_array($srcMap)) {
    fwrite(STDERR, "Bad src JSON: $src\n");
    exit(4);
}
...
function http_get_json($url, $authKey)
{
    ...
    if (!is_array($j)) {
        throw new Exception("Bad JSON from $url");
    }
    return $j;
}
...
function get_target_languages($languagesEndpoint, $authKey)
{
    ...
    foreach ($arr as $item) {
        if (isset($item['language'])) {
            $codes[] = strtolower($item['language']);
        }
    }
...
function translate_batch($endpoint, $authKey, $target_lang, $texts, $source_lang = null)
{
    ...
    if ($source_lang) {
        $scalars['source_lang'] = strtoupper($source_lang);
    }
...
if (!$targets) {
    fwrite(STDERR, "No target languages resolved.\n");
    exit(5);
}
