...
dashboard_mail_extract_email() {
  local value="${1:-}"
  value="$(_dashboard_trim "$value")"
  value="${value//$'\r'/ }"
  value="${value//$'\n'/ }"
  if [[ "$value" =~ \<([^>]+)\> ]]; then
  ...
  value="$(_dashboard_trim "$value")"
  if printf '%s\n' "$value" | grep -Eq '^[A-Za-z0-9.!#$%&'\''*+/=?^_`{|}~-]+@[A-Za-z0-9.-]+$'; then
    printf '%s' "$value"
  fi
}

dashboard_mail_default_sender() {
  printf '%s' "server-dashboard@localhost.invalid"
}
...
dashboard_env_bootstrap() {
  ...
  local cfg_mail_from=""
  local cfg_mail_envelope_from=""
  if [ -n "$cfg_file" ]; then
    cfg_mail_from="$(_dashboard_cfg_get "$cfg_file" '.mail.mail_from')"
    cfg_mail_envelope_from="$(_dashboard_cfg_get "$cfg_file" '.mail.mail_envelope_from')"
  fi

  if [ -z "${MAIL_FROM:-}" ]; then
    MAIL_FROM="$(_dashboard_trim "${cfg_mail_from:-}")"
  fi
  if [ -z "${MAIL_FROM:-}" ]; then
    MAIL_FROM="$(dashboard_mail_default_sender)"
  fi

  if [ -z "${MAIL_ENVELOPE_FROM:-}" ]; then
    MAIL_ENVELOPE_FROM="$(_dashboard_trim "${cfg_mail_envelope_from:-}")"
  fi
  if [ -z "${MAIL_ENVELOPE_FROM:-}" ]; then
    MAIL_ENVELOPE_FROM="$(dashboard_mail_extract_email "${MAIL_FROM:-}")"
  fi
  if [ -z "${MAIL_ENVELOPE_FROM:-}" ]; then
    MAIL_ENVELOPE_FROM="$(dashboard_mail_default_sender)"
  fi

  if [ -z "${REPORT_EMAIL_FROM:-}" ]; then
    REPORT_EMAIL_FROM="$MAIL_FROM"
  fi
  if [ -z "${REPORT_EMAIL_ENVELOPE_FROM:-}" ]; then
    REPORT_EMAIL_ENVELOPE_FROM="$MAIL_ENVELOPE_FROM"
  fi
  if [ -z "${AUDIT_EMAIL_FROM:-}" ]; then
    AUDIT_EMAIL_FROM="$MAIL_FROM"
  fi
  if [ -z "${AUDIT_EMAIL_ENVELOPE_FROM:-}" ]; then
    AUDIT_EMAIL_ENVELOPE_FROM="$MAIL_ENVELOPE_FROM"
  fi
  if [ -z "${BACKUP_ALERT_EMAIL_FROM:-}" ]; then
    BACKUP_ALERT_EMAIL_FROM="$MAIL_FROM"
  fi
  if [ -z "${BACKUP_ALERT_EMAIL_ENVELOPE_FROM:-}" ]; then
    BACKUP_ALERT_EMAIL_ENVELOPE_FROM="$MAIL_ENVELOPE_FROM"
  fi

  export STATE_OWNER REPORT_EMAIL_TO AUDIT_EMAIL BACKUP_ALERT_EMAIL
  export MAIL_FROM MAIL_ENVELOPE_FROM
  export REPORT_EMAIL_FROM REPORT_EMAIL_ENVELOPE_FROM
  export AUDIT_EMAIL_FROM AUDIT_EMAIL_ENVELOPE_FROM
  export BACKUP_ALERT_EMAIL_FROM BACKUP_ALERT_EMAIL_ENVELOPE_FROM
}
