...
EMAIL_FROM="${REPORT_EMAIL_FROM:-${MAIL_FROM:-server-dashboard@localhost.invalid}}"
EMAIL_ENVELOPE_FROM="${REPORT_EMAIL_ENVELOPE_FROM:-${MAIL_ENVELOPE_FROM:-$EMAIL_FROM}}"
...

{
  ...
  echo
  echo "--${boundary}--"
} | "$SENDMAIL" -t -i -f "$EMAIL_ENVELOPE_FROM"
