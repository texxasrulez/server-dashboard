...
LOC_DIR = os.path.join(ROOT, "assets", "i18n")
...
def flatten_map(node, prefix="", out=None):
    if out is None:
        out = {}
    if isinstance(node, dict):
        for key, val in node.items():
            ...
            flatten_map(val, path, out)
    else:
        if isinstance(node, str) and prefix:
            out[prefix] = node
    return out

def unflatten_map(flat):
    tree = {}
...
def load_locale(path):
    ...
    try:
        ...
        return {}
    return flatten_map(data)

def write_locale(path, mapping):
    tree = unflatten_map(mapping)
    _write(path, json.dumps(tree, ensure_ascii=False, indent=2) + "\n")
...
def main():
    ...
    ap.add_argument("--source", default="en.json", help="Source english file name")
    ...
    if args.all:
        targets = [loc for loc in DEEPL_CODE_MAP if os.path.exists(os.path.join(LOC_DIR, loc + ".json"))]
    else:
    ...
    for loc in targets:
        ...
        loc_path = os.path.join(LOC_DIR, loc + ".json")
        if not code:
