...
$PAGE_JS    = ['assets/js/utils/sortable-table.js', 'assets/js/pages/users.profile.js']; // page-scoped JS loaded via includes/foot.php
...
{
    $u = trim((string)$u);
    return $u !== '' && !filter_var($u, FILTER_VALIDATE_URL) ? null : $u;
}
function sanitize_email($e)
{
    $e = trim((string)$e);
    return $e !== '' && !filter_var($e, FILTER_VALIDATE_EMAIL) ? null : $e;
}

/* admin-only actions */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['admin_panel'])) {
    if (!csrf_check($_POST['csrf'] ?? '')) {
        $aerr = 'Invalid session.';
    } elseif (!$isAdmin) {
        $aerr = 'Admin only.';
    } else {
        ...
        try {
            if ($act === 'test_storage') {
                $data = users_load();
                users_save($data);
                $amsg = 'Storage test write OK.';
            } elseif ($act === 'add_user') {
                [$ok,$msgx] = user_add($_POST['username'] ?? '', $_POST['password'] ?? '', ($_POST['role'] ?? 'user'));
                $ok ? $amsg = $msgx : $aerr = $msgx;
            } elseif ($act === 'delete_user') {
                [$ok,$msgx] = user_delete($_POST['username'] ?? '');
                $ok ? $amsg = $msgx : $aerr = $msgx;
            } elseif ($act === 'set_role') {
                [$ok,$msgx] = user_set_role($_POST['username'] ?? '', $_POST['role'] ?? 'user');
                $ok ? $amsg = $msgx : $aerr = $msgx;
            }
        } catch (Throwable $e) {
            $aerr = $e->getMessage();
...
if (isset($_POST['action']) && $_POST['action'] === 'save_profile') {
    if (!csrf_check($_POST['csrf'] ?? '')) {
        ...
        if (!$err && in_array(null, [$site,$tw,$gh,$mdn,$li,$avatar], true)) {
            $err = 'One or more links are invalid. Use full URLs (https://…).';
        }
        ...
        if (!$err) {
            ...
            foreach ($data['users'] as &$u) {
                if (strtolower($u['username']) === $un) {
                    if (!isset($u['profile']) || !is_array($u['profile'])) {
                        $u['profile'] = [];
                    }
...
if (isset($_POST['action']) && $_POST['action'] === 'change_pass') {
    if (!csrf_check($_POST['csrf'] ?? '')) {
        ...
        if ($n1 === '' || $n1 !== $n2) {
            ...
            if (!$record || !password_verify($cur, $record['password_hash'])) {
                $err = 'Current password is incorrect.';
            } else {
                $data = users_load();
                foreach ($data['users'] as &$uu) {
                    if (strtolower($uu['username']) === strtolower($me['username'])) {
                        $uu['password_hash'] = password_hash($n1, PASSWORD_DEFAULT);
                        break;
                    }
                }
                unset($uu);
...
if (!isset($p['socials'])) {
    $p['socials'] = ['twitter' => '','github' => '','mastodon' => '','linkedin' => ''];
}
?>
<div class="card">
  ...
  <div class="card users-admin">
    ...
    <div class="table-wrap">
      <table class="table js-sortable">
        <thead>
          <tr class="muted"><th data-sort="str">Username</th><th data-sort="str">Role</th><th class="actions" data-sort="str">Actions</th></tr>
        </thead>
        <tbody>
